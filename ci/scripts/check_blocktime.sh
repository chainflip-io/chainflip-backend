...
SLOT_DURATION="$(curl -H "Content-Type: application/json" -d '{ "jsonrpc":"2.0", "id":"1", "method":"cf_slot_duration", "params":{} }' http://localhost:9944 | jq '.result')"

# we expect a block time of 6000ms on all live networks. ANYTHING ELSE WILL BREAK THE NETWORK.
if [[ $SLOT_DURATION == 6000 ]]; then
