...
use crate::{
	...
	test_all_crypto_schemes, ChainSigning, CryptoScheme, Rng,
};
...
/// for a given scheme
fn test_signing_for_scheme<C: ChainSigning>() {
	...
	type Point<C> = <<C as ChainSigning>::CryptoScheme as CryptoScheme>::Point;
	for _ in 0..10 {
		let secret_key = <Point<C> as ECPoint>::Scalar::random(&mut rng);
		let public_key = <Point<C> as ECPoint>::from_scalar(&secret_key);
		...
		let signature =
			generate_single_party_signature::<C::CryptoScheme>(&secret_key, &payload, &mut rng);

		// Verification is typically delegated to third-party libraries whose
		// behaviour we are attempting to replicate with FROST.
		assert!(C::CryptoScheme::verify_signature(&signature, &agg_key, &payload).is_ok());
	}
}
