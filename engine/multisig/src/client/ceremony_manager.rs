...
use lazy_static;
use prometheus::{IntCounterVec, Opts, Registry, IntGaugeVec};
lazy_static::lazy_static! {
	...
	pub static ref CEREMONY_PROCESSED_MSG: IntCounterVec = IntCounterVec::new(Opts::new("ceremony_msg", "Count all the messages for a give ceremony"), &["ceremonyId", "fromAccountId"]).expect("Metric succesfully created");
}

fn register_metrics(reg: Registry) {
	reg.register(Box::new(CEREMONY_MANAGER_BAD_MSG.clone()))
		.expect("Metric succesfully register");
	reg.register(Box::new(UNAUTHORIZED_CEREMONY.clone()))
		.expect("Metric succesfully register");
	reg.register(Box::new(CEREMONY_RUNNER_BAD_MSG.clone()))
		.expect("Metric succesfully register");
	reg.register(Box::new(CEREMONY_PROCESSED_MSG.clone()))
		.expect("Metric succesfully register");
}
		...
		Chain: ChainSigning<CryptoScheme = Ceremony::Crypto>,
	{
		...
		if !self.ceremony_handles.contains_key(&ceremony_id) {
			...
			if ceremony_id > latest_ceremony_id + Chain::CEREMONY_ID_WINDOW {
				...
				UNAUTHORIZED_CEREMONY.with_label_values(&[Chain::NAME]).set(total.try_into().unwrap());
