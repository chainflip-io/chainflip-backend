...
type PointFromChain<Chain> = <<Chain as ChainSigning>::CryptoScheme as CryptoScheme>::Point;
...
async fn test_sign_multiple_payloads<Chain: ChainSigning>(
	...
	let messages = run_stages!(
		signing_ceremony,
		messages,
		signing_data::VerifyComm2<PointFromChain<Chain>>,
		signing_data::LocalSig3<PointFromChain<Chain>>,
		signing_data::VerifyLocalSig4<PointFromChain<Chain>>
	);
	signing_ceremony.distribute_messages(messages).await;
...
async fn should_sign_with_different_keys() {
	...
	type Point = PointFromChain<Chain>;
