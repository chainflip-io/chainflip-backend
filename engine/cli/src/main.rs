...
async fn request_claim(
    ...
    let (_, block_stream, state_chain_client, _) = connect_to_state_chain(&settings.state_chain, false, logger).await.map_err(|_| anyhow::Error::msg("Failed to connect to state chain node. Please ensure your state_chain_ws_endpoint is pointing to a working node."))?;
...
async fn rotate_keys(settings: &CLISettings, logger: &slog::Logger) -> Result<()> {
    let (_, _, state_chain_client, _) = connect_to_state_chain(&settings.state_chain, false, logger).await.map_err(|e| anyhow::Error::msg(format!("{:?} Failed to connect to state chain node. Please ensure your state_chain_ws_endpoint is pointing to a working node.", e)))?;
