...
use crate::{
	...
	elections::vote_submitter::VoteSubmitter,
		...
		+ Sync,
{
	...
	let vote_submitter = VoteSubmitter::start(scope, state_chain_client.clone());

	let start_arb = super::arb_elections::start(
		scope,
		arb_client.clone(),
		state_chain_client.clone(),
		vote_submitter.clone(),
	);
	...
	let start_sol =
		super::sol::start(scope, sol_client, state_chain_client.clone(), vote_submitter.clone());

	let start_btc =
		super::btc::start(scope, btc_client, state_chain_client.clone(), vote_submitter.clone());
