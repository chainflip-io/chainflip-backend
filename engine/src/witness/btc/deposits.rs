...
pub mod tests {
	...
	fn deposit_witnesses_highest_10_utxos() {
		...
		pairs.extend(amounts.iter().map(|amount| (amount.clone(), &address)));
		...
		for i in 0..10 {
			assert_eq!(deposit_witnesses[i].amount, UTXO_WITNESSED_1 + 9 - (i as u64));
			assert_eq!(deposit_witnesses[i].deposit_address, address.script_pubkey());
		}
	}
}
