...
impl ErrorDecoder {
	pub fn new() -> Self {
       ...
       let metadata: frame_support::metadata::RuntimeMetadataLastVersion = match metadata.1 {
            ...
            _ => { panic!("If this breaks change the version above to match new metadata version, and update the test below like you should have."); }
        };

        Self {
            errors: metadata.pallets.into_iter().filter_map(|pallet| {
                Some((
                    pallet.index,
                    (
                        pallet.name,
                        match &metadata.types.types[pallet.error?.ty.id as usize].ty.type_def {
                            scale_info::TypeDef::Variant(variant_type_def) => {
                                variant_type_def.variants.iter().map(|variant| {
                                    (variant.index, (variant.name.clone(), variant.docs.clone()))
    ...
    pub fn decode_dispatch_error(&self, dispatch_error: sp_runtime::DispatchError) -> DispatchError {
        match dispatch_error {
            sp_runtime::DispatchError::Module(module_error) => {
                if let Some((pallet, (name, error))) = u8::decode(&mut &module_error.error[..]).ok().and_then(|error_index| {
                    self.errors.get(&module_error.index).and_then(|(pallet, pallet_errors)| pallet_errors.get(&error_index).map(|error| (pallet, error)))
                }) {
...
mod tests {
    ...
    fn check_metadata_version() {
        ...
        match metadata.1 {
            ...
            _ => { panic!("If this breaks also change version above, to match new metadata version"); }
        }
    }
}
