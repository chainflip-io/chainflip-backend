...
pub mod utils {
	...
	use std::{collections::HashSet, fmt, result, sync::Arc};
	...
	pub fn new_json_logger_with_tag_filter(
		...
		.fuse();
		slog::Logger::root(
			slog_async::Async::new(drain).chan_size(ASYNC_SLOG_CHANNEL_SIZE).build().fuse(),
			o!(),
		)
	}
...
pub mod test_utils {
	...
	/// Prints an easy to read log and the list of key/values.
	/// Also creates a tag cache that collects all tags so you can later confirm a log was
	/// triggered.
	pub fn new_test_logger_with_tag_cache() -> (slog::Logger, TagCache) {
		let d1 = TagCache::default();
		let d2 = Fuse(PrintlnDrainVerbose);
		(slog::Logger::root(slog::Duplicate::new(d1.clone(), d2).fuse(), o!()), d1)
	}
	...
	pub fn new_test_logger_with_tag_cache_and_tag_filter(
		...
		.fuse();
		(slog::Logger::root(slog::Duplicate::new(drain1, drain2).fuse(), o!()), tc)
	}
