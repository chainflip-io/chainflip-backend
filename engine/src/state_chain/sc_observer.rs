...
mod tests {
    ...
    // this definitely needs to have some static tests against real data from the chain.
    fn storage_change_set_encoded_from_types<StorageType: Encode>(
        change: StorageType,
        block: state_chain_runtime::Hash,
        storage_key: StorageKey,
    ) -> StorageChangeSet<state_chain_runtime::Hash> {
        let storage_data: StorageData = StorageData(StorageType::encode(&change));
        let changes: Vec<(StorageKey, Option<StorageData>)> =
            vec![(storage_key, Some(storage_data))];
        StorageChangeSet { block, changes }
    }

    #[test]
    fn storage_change_set_encoding_works() {
        // let change_set_from_chain = // let's say account data. good amount of complexity, and often queried for

        // compare to that generated by passing in the AccountInfo type with the values.
