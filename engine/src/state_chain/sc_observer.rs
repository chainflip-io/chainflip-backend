...
use pallet_cf_vaults::{BlockHeightWindow, CeremonyId};
use slog::o;
use sp_core::H256;
    ...
    loop {
        tokio::select!(
            option_outcome = multisig_outcome_receiver.recv() => {
                match option_outcome {
                    Some(outcome) => match outcome {
                        MultisigOutcome::Keygen(KeygenOutcome { id, result }) => {
                            let chain_id = keygen_chain_ids.remove(&id).expect("Chain id should be recorded on request");
            ...
            option_result_block_header = sc_block_stream.next() => match option_result_block_header {
                Some(result_block_header) => match result_block_header {
                    Ok(block_header) => {
                                ...
                                for (_phase, event, _topics) in events {
                                    match event {
                                        ...
                                        state_chain_runtime::Event::Vaults(
                                            ...
                                            keygen_chain_ids.insert(ceremony_id, chain_id);
...
mod tests {
    ...
    use crate::{
        eth::{EthWsRpcClient, MockEthRpcApi},
