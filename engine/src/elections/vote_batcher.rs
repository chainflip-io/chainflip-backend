...
//! submits them as a single `Environment::submit_election_votes` extrinsic.
//!
...
use state_chain_runtime::{chainflip::ElectionInstancesVotesBatch, Runtime};
...
pub struct VoteBatcher {
	votes_sender: mpsc::Sender<ElectionInstancesVotesBatch>,
}
...
impl VoteBatcher {
	...
	pub fn send(&self, instance: &'static str, votes: ElectionInstancesVotesBatch) {
...
struct PendingBatches {
	batches: Vec<ElectionInstancesVotesBatch>,
...
impl PendingBatches {
	...
	fn insert(&mut self, votes: ElectionInstancesVotesBatch, now: tokio::time::Instant) {
		let mut votes = votes;
		for batch in self.batches.iter_mut() {
			match batch.try_merge(votes) {
				Ok(()) => {
					votes = ElectionInstancesVotesBatch::default();
					break
				},
	...
	fn take(&mut self) -> Vec<ElectionInstancesVotesBatch> {
		self.hard_cap = None;
		self.deadline = None;
...
async fn run<StateChainClient: SignedExtrinsicApi + Send + Sync + 'static>(
	mut receiver: mpsc::Receiver<ElectionInstancesVotesBatch>,
	state_chain_client: Arc<StateChainClient>,
) {
	...
	for batch in batches {
		state_chain_client
			.submit_signed_extrinsic(
				pallet_cf_environment::Call::<Runtime>::submit_election_votes {
					votes: Box::new(batch),
				},
			)
			.await;
...
mod tests {
	...
	fn bitcoin_votes() -> ElectionInstancesVotesBatch {
		<Instance3 as BatchedInstance>::votes(BTreeMap::new().try_into().unwrap())
	}

	fn solana_votes() -> ElectionInstancesVotesBatch {
		<Instance5 as BatchedInstance>::votes(BTreeMap::new().try_into().unwrap())
	}

	fn tron_votes() -> ElectionInstancesVotesBatch {
