...
impl CommitmentConfig {
	#[deprecated(since = "1.5.5", note = "Please use CommitmentConfig::processed() instead")]
	pub fn recent() -> Self {
		Self { commitment: CommitmentLevel::Recent }
	}

	#[deprecated(since = "1.5.5", note = "Please use CommitmentConfig::finalized() instead")]
	pub fn max() -> Self {
		Self { commitment: CommitmentLevel::Max }
	}

	#[deprecated(since = "1.5.5", note = "Please use CommitmentConfig::finalized() instead")]
	pub fn root() -> Self {
		Self { commitment: CommitmentLevel::Root }
	}

	#[deprecated(since = "1.5.5", note = "Please use CommitmentConfig::confirmed() instead")]
	pub fn single() -> Self {
		Self { commitment: CommitmentLevel::Single }
	}

	#[deprecated(since = "1.5.5", note = "Please use CommitmentConfig::confirmed() instead")]
	pub fn single_gossip() -> Self {
		Self { commitment: CommitmentLevel::SingleGossip }
	}
	...
	pub fn is_processed(&self) -> bool {
		matches!(&self.commitment, CommitmentLevel::Processed | CommitmentLevel::Recent)
	}
...
pub enum CommitmentLevel {
	/// (DEPRECATED) The highest slot having reached max vote lockout, as recognized by a
	/// supermajority of the cluster.
	#[deprecated(since = "1.5.5", note = "Please use CommitmentLevel::Finalized instead")]
	Max,

	/// (DEPRECATED) The highest slot of the heaviest fork. Ledger state at this slot is not
	/// derived from a finalized block, but if multiple forks are present, is from the fork the
	/// validator believes is most likely to finalize.
	#[deprecated(since = "1.5.5", note = "Please use CommitmentLevel::Processed instead")]
	Recent,
	...
	/// (DEPRECATED) The highest slot having reached 1 confirmation by supermajority of the
	/// cluster.
