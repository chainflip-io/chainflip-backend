    ...
    multisig::{
        ...
        client::{
            ...
            tests::helpers::{
                ...
                standard_signing, standard_signing_coroutine, KeygenCeremonyRunner,
...
async fn should_delay_rts_until_key_is_ready() {
    // Pre-run a deterministic ceremony to get the key id that will be generated by a successful keygen
    let keygen_ceremony = KeygenCeremonyRunner::new_with_default();
    ...
    let [test_node_id] = &keygen_ceremony.select_account_ids();
...
async fn pending_rts_should_expire() {
    ...
    let [target_account_id] = &signing_ceremony.select_account_ids();
    ...
    assert_eq!(
        outcome,
        client::SigningOutcome::timeout(signing_ceremony.ceremony_id, vec![])
    );
}
