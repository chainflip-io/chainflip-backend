...
pub struct AwaitCommitments1<P: ECPoint> {
    ...
    signing_common: SigningStateCommonInfo<P>,
    nonces: Box<SecretNoncePair<P>>,
}

impl<P: ECPoint> AwaitCommitments1<P> {
    pub fn new(mut common: CeremonyCommon, signing_common: SigningStateCommonInfo<P>) -> Self {
...
impl<Crypto: CryptoScheme> BroadcastStageProcessor<SigningCeremony<Crypto>>
    for AwaitCommitments1<Crypto::Point>
{
    ...
    async fn process(
        ...
        let processor = VerifyCommitmentsBroadcast2::<Crypto::Point> {
...
#[async_trait]
impl<Crypto: CryptoScheme> BroadcastStageProcessor<SigningCeremony<Crypto>>
    for VerifyCommitmentsBroadcast2<Crypto::Point>
{
    ...
    async fn process(
            ...
            Err((reported_parties, abort_reason)) => {
                return SigningStageResult::Error(
                    ...
                    CeremonyFailureReason::BroadcastFailure(
                        abort_reason,
                        <Self as BroadcastStageProcessor<SigningCeremony<Crypto>>>::NAME,
                    ),
                );
            }
        };
        ...
        let processor = LocalSigStage3::<Crypto::Point> {
            common: self.common.clone(),
...
struct LocalSigStage3<P: ECPoint> {
    common: CeremonyCommon,
    signing_common: SigningStateCommonInfo<P>,
    // Our nonce pair generated in the previous stage
    nonces: Box<SecretNoncePair<P>>,
    // Public nonce commitments (verified)
    commitments: BTreeMap<AuthorityCount, Comm1<P>>,
}

derive_display_as_type_name!(LocalSigStage3<P: ECPoint>);

#[async_trait]
impl<Crypto: CryptoScheme> BroadcastStageProcessor<SigningCeremony<Crypto>>
    for LocalSigStage3<Crypto::Point>
{
    ...
    async fn process(
        ...
        let processor = VerifyLocalSigsBroadcastStage4::<Crypto::Point> {
            common: self.common.clone(),
            signing_common: self.signing_common.clone(),
...
struct VerifyLocalSigsBroadcastStage4<P: ECPoint> {
    common: CeremonyCommon,
    signing_common: SigningStateCommonInfo<P>,
    ...
    local_sigs: BTreeMap<AuthorityCount, Option<LocalSig3<P>>>,
}

derive_display_as_type_name!(VerifyLocalSigsBroadcastStage4<P: ECPoint>);

#[async_trait]
impl<Crypto: CryptoScheme> BroadcastStageProcessor<SigningCeremony<Crypto>>
    for VerifyLocalSigsBroadcastStage4<Crypto::Point>
{
