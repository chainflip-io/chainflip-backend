...
use client::{
    CeremonyAbortReason, CeremonyOutcomeResult, EventSender, InnerEvent, KeygenResultInfo,
    MultisigMessage, SchnorrSignature,
};
                    ...
                    ProcessMessageResult::CollectedAll => {
                        ...
                        match state.finalize() {
                            ...
                            StageResult::Error(bad_validators) => {
                                ...
                                authorised_state.send_result(Err((
                                    CeremonyAbortReason::Invalid,
                                    blamed_parties,
                                )));
                            }
                            StageResult::Done(result) => {
                                authorised_state.send_result(Ok(result));
