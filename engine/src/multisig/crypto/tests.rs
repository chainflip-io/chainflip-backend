use crate::multisig::{
	...
	CryptoScheme, KeyId, Rng, client::KeygenResult,
};
...
fn test_signing_for_scheme<C: CryptoScheme>() {
	...
	// Running this multiple times will ensure that we produce various keys with potential incompatibilities that should get fixed by the code
	// This applies mostly to Ethereum and Bitcoin keys, where not every random private key is valid.
	for _ in 0..10 {
		...
		let my_keygen_result: KeygenResult<C> = KeygenResult::new_compatible(my_key_share, vec![public_key]);
		...
		assert!(C::verify_signature(&signature, &KeyId(public_key.as_bytes().to_vec()), &payload).is_ok());
	}
}
