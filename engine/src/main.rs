			...
			loop {
				...
				match cfe_status {
					CfeStatus::Active(_) =>
						if !compatible {
							tracing::info!(
								"Runtime version ({current_release_version:?}) is no longer compatible, shutting down the engine!"
							);
					...
					CfeStatus::Idle =>
						if compatible {
							...
							tracing::info!("Runtime version ({current_release_version:?}) is compatible, starting the engine!");
