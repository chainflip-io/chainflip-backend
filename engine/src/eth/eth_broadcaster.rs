...
pub async fn start<M: IMQClient + Send + Sync>(settings: settings::Settings) -> anyhow::Result<()> {
...
impl<M: IMQClient + Send + Sync> EthBroadcaster<M> {
    ...
    pub async fn run(&self) -> Result<()> {
        // TODO: Use the right type for the subscription, not string
        let stream = self
            .mq_client
...
mod tests {
    ...
    #[tokio::test]
    #[ignore = "requires fresh eth node setup with `chainflip` mnemonic"]
    async fn test_eth_broadcast_fail() {
        let eth_broadcaster = new_eth_broadcaster::<NatsMQClient>().await.unwrap();
