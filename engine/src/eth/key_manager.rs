...
impl EthObserver for KeyManager {
        ...
        EthRpcClient: EthRpcApi + Sync + Send,
    {
        ...
        match event.event_parameters {
            ...
            KeyManagerEvent::GovernanceAction { call_hash } => {
                let _result = state_chain_client
                    .submit_signed_extrinsic(
                        pallet_cf_witnesser::Call::witness {
                            call: Box::new(
                                pallet_cf_governance::Call::set_whitelisted_call_hash { call_hash }
                                    .into(),
                            ),
                        },
                        logger,
            ...
            move |event_signature: H256, raw_log: RawLog| -> Result<KeyManagerEvent> {
                Ok(if event_signature == ak_nonce_consumers_set.signature {
                    ...
                    KeyManagerEvent::GovernanceAction {
                        call_hash: utils::decode_log_param(&log, "message")?,
                    }
                } else {
...
mod tests {
    ...
    fn test_gov_action_parsing() {
        ...
        {
            KeyManagerEvent::GovernanceAction { call_hash } => {
                assert_eq!(call_hash, H256::from_low_u64_be(42069).as_ref());
            }
