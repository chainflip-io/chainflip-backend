...
impl BlockWitnesserGenerator for EthBlockWitnesserGenerator {
	...
	async fn get_block_stream(
		...
		let block_stream =
			safe_block_subscription_from(from_block, self.ws_rpc.clone(), self.http_rpc.clone())
				...
				.map_err(|err| {
					...
					err.context("Eth witnesser failed to get block stream.")
				})?;
		...
		.map_err(|elapsed| {
			anyhow::anyhow!("Timed out: Time elapsed {elapsed}")
				.context("Eth witnesser failed to get latest block stream.")
		});
