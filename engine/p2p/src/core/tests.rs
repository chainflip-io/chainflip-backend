...
/// The p2p OS threads must keep the subscriber of the thread that spawned them.
///
/// This is a regression test for a failure that is completely silent: the engine
/// installs its subscriber with `set_default()` (thread-local), so a bare
/// `std::thread::spawn` starts with no subscriber and every line it logs simply
/// never exists — no error, no dropped-message count, and no amount of `RUST_LOG`
/// brings it back. It hid all p2p connect/disconnect events, the only record of
/// connection state the engine produces.
#[test]
fn spawned_threads_keep_the_tracing_subscriber() {
	use std::sync::{
	...
	/// Counts events. That is all this needs to distinguish "the thread has a
	/// subscriber" from "its output is silently discarded".
	...
	super::spawn_with_tracing("test-p2p-thread", || {
		tracing::info!("emitted from a spawned p2p thread");
	})
	.join()
	.unwrap();
	...
	assert_eq!(
		...
		1,
		"the spawned thread's log was discarded: the subscriber was not propagated",
	);
}
