...
build-localnet() {
  ...
  while ! curl --user flip:flip -H 'Content-Type: text/plain;' -d '{"jsonrpc":"1.0", "id": "1", "method": "getblockchaininfo", "params" : []}' -v http://127.0.0.1:8332 ; do
    echo "🪙 Waiting for Bitcoin node to start"
    docker logs bitcoin || true
    sleep 5
  done
