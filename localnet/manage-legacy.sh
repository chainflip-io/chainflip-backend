...
build-localnet() {
  ...
  if [ "$OS_TYPE" == "Linux" ]; then
    ...
    sudo cp ./old-engine-dylib/libchainflip_engine_v*.so $BINARY_ROOT_PATH/
  else
  ...
  docker-compose -f localnet/docker-compose.yml -p "chainflip-localnet" up -d $CORE_CONTAINERS $additional_docker_compose_up_args >>$DEBUG_OUTPUT_DESTINATION 2>&1
  ...
  docker-compose -f localnet/docker-compose.yml -p "chainflip-localnet" up -d $ARB_CONTAINERS $additional_docker_compose_up_args >>$DEBUG_OUTPUT_DESTINATION 2>&1
  echo "🪄 Deploying L2 Contracts"
  docker-compose -f localnet/docker-compose.yml -p "chainflip-localnet" up -d arb-init $additional_docker_compose_up_args >>$DEBUG_OUTPUT_DESTINATION 2>&1
