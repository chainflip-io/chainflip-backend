...
fn group_outputs_by_quote(outputs: &[OutputTx]) -> Vec<(Uuid, Vec<OutputTx>)> {
    // Make sure we only have valid outputs and group them by the quote
    let valid_txs = outputs.iter().filter(|tx| tx.coin == Coin::ETH);
...
impl<E: EthereumClient + Sync + Send> OutputSender for EthOutputSender<E> {
    async fn send<T: TransactionProvider + Sync>(
        ...
        let grouped = group_outputs_by_quote(outputs);
...
mod test {
    ...
    fn test_group_outputs_by_quote() {
        ...
        let mut eth_output_3 = create_fake_output_tx(Coin::ETH);
        let mut eth_output_4 = create_fake_output_tx(Coin::ETH);

        let quote_1 = uuid::Uuid::new_v4();
        eth_output_1.quote_tx = quote_1;
        eth_output_3.quote_tx = quote_1;

        let quote_2 = uuid::Uuid::new_v4();
        eth_output_2.quote_tx = quote_2;
        eth_output_4.quote_tx = quote_2;
        ...
        assert_eq!(first.1, vec![eth_output_1, eth_output_3]);

        let second = result.iter().find(|(quote, _)| quote == &quote_2).unwrap();
        assert_eq!(second.0, quote_2);
        assert_eq!(second.1, vec![eth_output_2, eth_output_4]);
    }
