...
/// Re-export of `scale_decode::DecodeAsType` and `serde_json::Value`.
pub mod ext {
...
/// Visitor that delegates decoding to `RawJsonVisitor` and wraps the result in
/// `ScaleDecodedToJson`.
...
/// byte arrays: If each item in the array decodes to a json Number value, and each item was encoded
/// as a single byte, then the array is interpreted as a byte array.
#[derive(Debug)]
...
fn trimmed_hex<T: AsRef<[u8]>>(bytes: T) -> String {
	...
	let short = s.trim_start_matches('0');
	format!("0x{}", if short.is_empty() { "0" } else { short })
}
		...
		$(
			...
			_type_id: &scale_decode::visitor::TypeIdFor<Self>,
				...
				Ok(serde_json::Value::Array(
					values.map(|res| {
						res.and_then(|item| {
							Ok(item.decode_with_visitor(self)?)
						})
	...
	(
		...
		$(
			...
			_type_id: &scale_decode::visitor::TypeIdFor<Self>,
				...
				// - Each value decodes to an unsigned json Number (ie. not an enum)
				// - AND each value was encoded as a single byte
				let (is_bytes, all_values) = values.try_fold(
					(true, Vec::with_capacity(values.remaining())),
