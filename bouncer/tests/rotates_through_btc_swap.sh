...
MY_ADDRESS=`pnpm tsx ./commands/new_dot_address.ts foo`
echo "Generated DOT address " $MY_ADDRESS
pnpm tsx ./commands/new_swap.ts btc dot $MY_ADDRESS 100
SWAP_ADDRESS=`pnpm tsx ./commands/observe_events.ts --timeout 10000 --succeed_on swapping:SwapDepositAddressReady --fail_on foo:bar | jq  -r ".data.depositAddress.Btc"`
./tests/rotates_vaults.sh
OLD_BALKANCE=`pnpm tsx ./commands/get_dot_balance.ts $MY_ADDRESS`
pnpm tsx ./commands/send_btc.ts $SWAP_ADDRESS 1
...
SWAP_ADDRESS=`pnpm tsx ./commands/observe_events.ts --timeout 10000 --succeed_on swapping:SwapDepositAddressReady --fail_on foo:bar | jq  -r ".data.depositAddress.Dot"`
OLD_BALANCE=`pnpm tsx ./commands/get_btc_balance.ts $MY_ADDRESS`
pnpm tsx ./commands/send_dot.ts $SWAP_ADDRESS 1000
pnpm tsx ./commands/observe_events.ts --timeout 60000 --succeed_on swapping:SwapExecuted --fail_on foo:bar > /dev/null
for i in `seq 60`; do
