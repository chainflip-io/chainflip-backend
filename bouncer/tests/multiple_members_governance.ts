...
import type { PalletCfGovernanceVotingAuthority } from 'generated/chaintypes/chainflip-node';
import { AccountId32 } from 'dedot/codecs';

// The governance members are a tree of groups, so collect the individuals at its leaves.
function flattenMembers(authority: PalletCfGovernanceVotingAuthority): string[] {
  switch (authority.type) {
    case 'Individual':
      return [authority.value.id.address(2112)];
    case 'SimpleGroup':
      return authority.value.members.flatMap(flattenMembers);
    case 'WeightedGroup':
      return authority.value.members.flatMap(([, member]) => flattenMembers(member));
    default:
      throw new Error(`unknown voting authority: ${JSON.stringify(authority)}`);
  }
}

function simpleGroup(members: string[], threshold: number): PalletCfGovernanceVotingAuthority {
  return {
    type: 'SimpleGroup',
    value: {
      threshold,
...
async function addAliceToGovernance<A = []>(cf: ChainflipIO<A>, initMembers: string[] = []) {
  ...
  await cf.submitGovernance({
    extrinsic: (api) => api.tx.governance.setVotingAuthority(simpleGroup(newMembers, newThreshold)),
  });
...
async function submitWithMultipleGovernanceMembers<A = []>(cf: ChainflipIO<A>) {
  ...
  const proposalId = await submitGovernanceExtrinsic(
    (chainflip) => chainflip.tx.governance.setVotingAuthority(simpleGroup([snowWhite.address], 1)),
    cf.logger,
  );
