...
export async function setupSwaps(logger: Logger): Promise<void> {
  ...
  await Promise.all([
    ...
    createLpPool(logger, 'SolUsdt', price.get('SolUsdt')!),
  ...
  const lp1Deposits = Promise.all([
    ...
    depositLiquidity(logger, 'Wbtc', deposits.get('Wbtc')!, false, '//LP_1'),
    ...
    depositLiquidity(logger, 'ArbUsdt', deposits.get('ArbUsdt')!, false, '//LP_1'),
    depositLiquidity(logger, 'Sol', deposits.get('Sol')!, false, '//LP_1'),
    depositLiquidity(logger, 'SolUsdc', deposits.get('SolUsdc')!, false, '//LP_1'),
    depositLiquidity(logger, 'SolUsdt', deposits.get('SolUsdt')!, false, '//LP_1'),
