...
export async function rangeOrder(ccy: Asset, amount: number){
	...
	const liquidity = BigInt(Math.round(current_sqrt_price/Math.pow(2,96)*Number(fine_amount)));
    ...
    await lpMutex.runExclusive(async () => {
    	await chainflip.tx.liquidityPools.collectAndMintRangeOrder(ccy.toLowerCase(), [-887272, 887272], liquidity).signAndSend(lp, {nonce: -1}, handleSubstrateError(chainflip));
    });
    await event;
}
