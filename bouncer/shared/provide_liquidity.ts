...
import { observeEvent, getAddress, getChainflipApi, encodeDotAddressForContract, assetToChain, handleSubstrateError, encodeBtcAddressForContract } from '../shared/utils';
...
export async function provideLiquidity(ccy: Asset, amount: number){
    ...
    if((await chainflip.query.liquidityProvider.emergencyWithdrawalAddress(lp.address, assetToChain(ccy))).toJSON() === null){
        let emergencyAddress = await getAddress(ccy, 'LP_1');
        emergencyAddress = ccy === 'DOT' ? encodeDotAddressForContract(emergencyAddress) : emergencyAddress;
	...
	let eventHandle = chain.get(ccy) === 'eth' ?  
	...
	let ingress_address = chain.get(ccy) === 'eth' ? (await eventHandle).depositAddress.toJSON() : (await eventHandle).depositAddress.toJSON()[chain.get(ccy)!];
	if(ccy == "BTC"){
