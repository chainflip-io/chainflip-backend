...
impl_runtime_apis! {
	...
	impl crate::runtime_apis::monitoring_api::MonitoringRuntimeApi<Block> for Runtime {
		...
		fn cf_external_chains_block_height() -> ExternalChainsBlockHeight {
			...
			let bsc = pallet_cf_chain_tracking::CurrentChainState::<Runtime, BscInstance>::get().unwrap();
		...
		fn cf_pending_broadcasts_count() -> PendingBroadcasts {
			PendingBroadcasts {
				...
				bsc: pallet_cf_broadcast::PendingBroadcasts::<Runtime, BscInstance>::decode_non_dedup_len().unwrap_or(0) as u32,
			}
		}
		...
		fn cf_open_deposit_channels_count() -> OpenDepositChannels {
			...
			OpenDepositChannels{
				...
				bsc: open_channels::<pallet_cf_chain_tracking::Pallet<Runtime, BscInstance>, BscInstance>(),
			}
		}
		fn cf_fee_imbalance() -> FeeImbalance<AssetAmount> {
			FeeImbalance {
				...
				bsc: pallet_cf_asset_balances::Pallet::<Runtime>::vault_imbalance(ForeignChain::Bsc.gas_asset()),
			}
		}
		...
		fn cf_rotation_broadcast_ids() -> ActivateKeysBroadcastIds{
			ActivateKeysBroadcastIds{
				...
				bsc: pallet_cf_broadcast::IncomingKeyAndBroadcastId::<Runtime, BscInstance>::get().map(|val| val.1),
			}
		}
	...
	impl crate::runtime_apis::elections_api::ElectoralRuntimeApi<Block> for Runtime {
		...
		fn cf_tron_electoral_data(account_id: AccountId) -> Vec<u8> {
			TronElections::electoral_data(&account_id).encode()
		}

		fn cf_tron_filter_votes(account_id: AccountId, proposed_votes: Vec<u8>) -> Vec<u8> {
			TronElections::filter_votes(&account_id, Decode::decode(&mut &proposed_votes[..]).unwrap_or_default()).encode()
		}
	...
	impl runtime_apis::custom_api::CustomRuntimeApi<Block> for Runtime {
		...
		fn cf_validator_info(account_id: &AccountId) -> ValidatorInfo {
			...
			ValidatorInfo {
				...
				bid: pallet_cf_validator::Pallet::<Runtime>::validator_bid(account_id),
			}
		}
		...
		fn cf_get_open_deposit_channels(account_id: Option<<Runtime as frame_system::Config>::AccountId>) -> ChainAccounts {
			...
			ChainAccounts {
				chain_accounts: [
					...
					open_deposit_channels_for_account::<Runtime, BscInstance>(account_id.as_ref()),
		...
		fn cf_all_open_deposit_channels() -> Vec<OpenedDepositChannels> {
			...
			let bsc_chain_accounts = open_deposit_channels_for_chain_instance::<Runtime, BscInstance>();
			...
			accounts.into_iter().map(|key| {
				...
				(account_id, channel_action_type, ChainAccounts {
					chain_accounts: [
						...
						bsc_chain_accounts.get(&key).cloned().unwrap_or_default(),
					].into_iter().flatten().collect()
				})
			}).collect()
		}
...
mod witnessed_events {
	...
	use cf_chains::{
		instances::{
			ArbitrumInstance, AssethubInstance, BitcoinInstance, EthereumInstance, TronInstance,
		},
		TransactionInId,
	};
	...
	pub fn extract_witnessed_events(
		...
		match chain {
			...
			ForeignChain::Assethub => extract_assethub_witnessed_events(),
	...
	fn extract_assethub_witnessed_events() -> Result<RawWitnessedEvents, DispatchErrorWithMessage> {
		let state =
			ElectoralUnsynchronisedState::<Runtime, AssethubInstance>::get().ok_or_else(|| {
				DispatchErrorWithMessage::RawMessage(
					b"Assethub electoral state not initialized".to_vec(),
				)
			})?;

		let deposits =
			extract_block_data!(&state.1, |h: &cf_chains::witness_period::BlockWitnessRange<
				Assethub,
			>| u64::from(*h.root()));
		let broadcasts =
			extract_block_data!(&state.2, |h: &cf_chains::witness_period::BlockWitnessRange<
				Assethub,
			>| u64::from(*h.root()));

		Ok(RawWitnessedEvents::Assethub { deposits, broadcasts })
	}
	...
	pub fn extract_ingress_events(
		...
		match chain {
			...
			ForeignChain::Assethub => {
				let state = ElectoralUnsynchronisedState::<Runtime, AssethubInstance>::get()
					.ok_or_else(|| {
						DispatchErrorWithMessage::RawMessage(
							b"Assethub electoral state not initialized".to_vec(),
						)
				...
				let deposits = extract_block_data!(
					&state.1,
					|h: &cf_chains::witness_period::BlockWitnessRange<Assethub>| u64::from(
						*h.root()
					)
				);
				Ok(IngressEvents {
