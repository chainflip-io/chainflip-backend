...
trait Num: Sized {
    type Prev;
    type Rec<T: TypeInfo, Z: At<Zero, T>, S: SucMap<T>>: At<Self, T>;
}
trait At<N: Num, T: TypeInfo> {
    type Result: Typed<T>;
}
trait TypeInfo {
...
trait SucMap<T: TypeInfo> {
    type Get<N: Num, X: At<N, T>>: At<Suc<N>, T>;
}

struct Suc<N>(sp_std::marker::PhantomData<N>);
impl<N: Num> Num for Suc<N> { 
    type Prev = N;
    type Rec<T: TypeInfo, Z: At<Zero, T>, S: SucMap<T>> = S::Get<N, N::Rec<T, Z, S>>;
}
struct Zero;
impl Num for Zero {
    type Prev = Zero;
    type Rec<T: TypeInfo, Z: At<Zero, T>, S: SucMap<T>> = Z; 
}
...
struct Impl<Ty>(Ty);
impl<T: TypeInfo, A: Typed<T>, As> At<Zero, T> for Impl<HCons<A,As>> {
    type Result = A;
}
impl<T: TypeInfo, A, As: At<N, T>, N: Num> At<Suc<N>, T> for Impl<HCons<A,As>> {
    type Result = As::Result;
}
struct HCons1<A>(A);
impl<A, T: TypeInfo> SucMap<T> for HCons1<A> {
    type Get<N: Num, X: At<N, T>> = Impl<HCons<A, X>>;
}
impl<A, As, N: Num, T: TypeInfo> At<N, T> for HCons<A,As> {
    type Result = <N::Rec<T, Impl<Self>, HCons1<A>> as At<N,T>>::Result;
}
trait Indexed {
    type Get<N: Num>;
}
...
impl<A: Typed<()>,As> Indexed for HCons<A,As> {
