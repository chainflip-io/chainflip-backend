...
impl OnRuntimeUpgrade for ClearSolanaEgressWitnessingElections {
	...
	fn pre_upgrade() -> Result<Vec<u8>, sp_runtime::TryRuntimeError> {
		let count_egress_witnessing: u64 = ElectionProperties::<Runtime, SolanaInstance>::iter_keys()
		...
		let count_nonce_witnessing: u64 = ElectionProperties::<Runtime, SolanaInstance>::iter_keys()
	...
	fn on_runtime_upgrade() -> Weight {
		...
		let next_election_id = if genesis_hashes::genesis_hash::<Runtime>() == genesis_hashes::PERSEVERANCE { 
			9_397_949u64 
		} else { 
			9_346_659u64 
		};
	...
	fn post_upgrade(state: Vec<u8>) -> Result<(), DispatchError> {
		let (count_egress_witnessing, count_nonce_witnessing) = <(u64, u64)>::decode(&mut state.as_slice())
			.map_err(|_| DispatchError::from("Failed to decode state"))?;

		// Verify no EgressWitnessing elections remain
		let remaining_egress_witnessing: u64 = ElectionProperties::<Runtime, SolanaInstance>::iter_keys()
			.filter(|id| matches!(id.extra(), CompositeElectionIdentifierExtra::D(_)))
			.count()
			.try_into()
			.unwrap();

		let remaining_nonce_witnessing: u64 = ElectionProperties::<Runtime, SolanaInstance>::iter_keys()
			.filter(|id| matches!(id.extra(), CompositeElectionIdentifierExtra::C(_)))
			.count()
			.try_into()
			.unwrap();
