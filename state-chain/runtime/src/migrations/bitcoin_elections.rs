...
impl OnRuntimeUpgrade for BitcoinElectionMigration {
	fn on_runtime_upgrade() -> Weight {
		...
		let maybe_old_vault_state = ElectoralUnsynchronisedState::<Runtime, BitcoinInstance>::get()
			.map(|composite| composite.2); // .2 is the 3rd field (VaultDepositWitnessing)

		if let Some(old_vault_state_bytes) = maybe_old_vault_state.map(|s| s.encode()) {
			let old_vault_state: old::OldBlockWitnesserState =
				codec::Decode::decode(&mut &old_vault_state_bytes[..])
					.expect("Failed to decode old vault state");
							...
							old_info
								...
								.map(|old_witness| {
									...
									let new_refund_params = ChannelRefundParameters {
										...
										refund_ccm_metadata: None, // <-- NEW FIELD
									};
									VaultDepositWitness {
			...
			let new_vault_state = BlockWitnesserState {
				...
				generate_election_properties_hook: old_vault_state
					.generate_election_properties_hook,
