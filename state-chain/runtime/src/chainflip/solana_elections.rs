...
#[cfg(test)]
mod tests {
	use electoral_systems::solana_swap_accounts_tracking::SolanaVaultSwapsVote;
	use pallet_cf_elections::electoral_system::{mock::DummyAccess, ConsensusVote, ConsensusVotes};

	use super::*;

	pub const NEW_ACCOUNT_1: SolAddress = SolAddress([1u8; 32]);
	pub const NEW_ACCOUNT_2: SolAddress = SolAddress([2u8; 32]);
	pub const NEW_ACCOUNT_3: SolAddress = SolAddress([3u8; 32]);

	pub const CLOSED_ACCOUNT_1: SolAddress = SolAddress([4u8; 32]);
	pub const CLOSED_ACCOUNT_2: SolAddress = SolAddress([5u8; 32]);

	pub const DUMMY_VALIDATOR: [u8; 32] = [0u8; 32];

	#[test]
	fn test_solana_vault_swaps_consensus() {
		let dummy_sol_swap = SolanaVaultSwapDetails {
			from: SolAsset::Sol,
			to: Asset::Btc,
			deposit_amount: Default::default(),
			destination_address: EncodedAddress::Btc(Default::default()),
			deposit_metadata: Default::default(),
			swap_account: Default::default(),
			creation_slot: Default::default(),
			broker_fees: Default::default(),
			refund_params: Default::default(),
			dca_params: Default::default(),
			boost_fee: Default::default(),
		};

		let new_account_1 = VaultSwapAccountAndSender {
			vault_swap_account: NEW_ACCOUNT_1,
			swap_sender: Default::default(),
		};
		let new_account_2 = VaultSwapAccountAndSender {
			vault_swap_account: NEW_ACCOUNT_2,
			swap_sender: Default::default(),
		};
		let new_account_3 = VaultSwapAccountAndSender {
			vault_swap_account: NEW_ACCOUNT_3,
			swap_sender: Default::default(),
		};

		let closed_account_1 = VaultSwapAccountAndSender {
			vault_swap_account: CLOSED_ACCOUNT_1,
			swap_sender: Default::default(),
		};
		let closed_account_2 = VaultSwapAccountAndSender {
			vault_swap_account: CLOSED_ACCOUNT_2,
			swap_sender: Default::default(),
		};

		let vote_1 = SolanaVaultSwapsVote {
			new_accounts: BTreeSet::from([
				(new_account_1, dummy_sol_swap.clone()),
				(new_account_2, dummy_sol_swap.clone()),
				(new_account_3, dummy_sol_swap.clone()),
			]),
		...
		assert_eq!(
			SolanaVaultSwapTracking::check_consensus(
				Default::default(),
				&DummyAccess::<SolanaVaultSwapTracking>::default(),
				None,
				ConsensusVotes {
					votes: (0..80usize)
						.map(|_| ConsensusVote {
							vote: Some(((), vote_1.clone())),
							validator_id: DUMMY_VALIDATOR.into(),
						})
						.chain((0..80usize).map(|_| ConsensusVote {
							vote: Some(((), vote_2.clone())),
							validator_id: DUMMY_VALIDATOR.into(),
						}))
						.collect::<Vec<_>>(),
				},
			...
			SolanaVaultSwapTracking::check_consensus(
				...
				ConsensusVotes {
					votes: (0..80usize)
						...
						.chain((0..80usize).map(|_| ConsensusVote {
							vote: Some(((), vote_3.clone())),
							validator_id: DUMMY_VALIDATOR.into(),
						}))
						.chain((0..80usize).map(|_| ConsensusVote {
							vote: Some(((), vote_4.clone())),
							validator_id: DUMMY_VALIDATOR.into(),
						}))
						.collect::<Vec<_>>(),
				},
		...
		assert_eq!(
			...
			.unwrap(),
			SolanaVaultSwapsVote {
				new_accounts: BTreeSet::from([(new_account_3, dummy_sol_swap.clone()),]),
