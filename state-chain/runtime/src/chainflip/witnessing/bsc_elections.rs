	...
	> for BscElectionHooks
{
	fn on_finalize(
		...
		if let Some(ingress_safety_margin) =
			pallet_cf_ingress_egress::WitnessSafetyMargin::<Runtime, BscInstance>::get()
		{
			pallet_cf_elections::ElectoralUnsynchronisedSettings::<Runtime, BscInstance>::mutate_extant(|settings|  {
