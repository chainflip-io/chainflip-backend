...
impl_runtime_apis! {
	...
	impl monitoring_apis::MonitoringRuntimeApi<Block> for Runtime {
		...
		fn cf_btc_discrepancy() -> i128 {
			pallet_cf_environment::BitcoinAvailableUtxos::<Runtime>::get().iter().fold(0, |acc, utxo| acc + utxo.amount) as i128 -
			pallet_cf_account_roles::AccountRoles::<Runtime>::iter().filter(|(_id, account_role)| *account_role == AccountRole::LiquidityProvider).map(|(id, _role)| id)
				.fold( AssetMap::from_fn(|_| 0u128), |acc, account| {
				acc.saturating_add(Self::cf_lp_total_balances(account.clone()).expect("All accounts are LPs, impossible to fail"))
			}).btc.btc as i128
		}
	}
