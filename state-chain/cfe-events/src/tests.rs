...
fn event_decoding() {
	...
	{
		check_encoding(
			CfeEvent::AuthoritiesUpdated {
				epoch_index: 9,
				authorities: vec![AccountId::from([1; 32]), AccountId::from([2; 32])],
			},
			"11090000000801010101010101010101010101010101010101010101010101010101010101010202020202020202020202020202020202020202020202020202020202020202",
		);
	}
}
