...
See "Profiling runtime execution" in the repo README for how to produce the log.
    ...
    ./state-chain/scripts/spans-to-profile.py --folded trace.log out.folded
    inferno-flamegraph out.folded > flame.svg    # cargo install inferno
...
extrinsic dispatches per instance. Whatever the log happens to contain is what gets
reported - sections with no matching spans are skipped.
...
def report(a, spans_in_block):
    ...
    # ---- extrinsic dispatches, by instance where the span carries a label ----
    calls = collections.defaultdict(lambda: [0, 0])
    for n in walk(roots):
        if n["name"] not in ("on_initialize", "on_finalize") and n["label"]:
            e = calls[(n["label"].replace("Elections", ""), n["name"])]
            e[0] += n["time"]
            e[1] += n["count"]
...
def write_folded(roots, dst):
    """Brendan Gregg folded stacks: `frame;frame;frame <self nanoseconds>`."""
    ...
    def walk(node, path):
        ...
        if node["label"]:
            label += f" [{node['label']}]"
        path = path + [label]
        folded[";".join(path)] += max(node["time"] - sum(k["time"] for k in node["kids"]), 0)
        for k in node["kids"]:
            walk(k, path)
    ...
    with open(dst, "w") as f:
        written = 0
        for stack, self_ns in sorted(folded.items()):
            if round(self_ns) > 0:
                f.write(f"{stack} {round(self_ns)}\n")
                written += 1
    return written
...
def main():
    ...
    ap.add_argument("--folded", action="store_true",
                    help="write Brendan Gregg folded stacks for inferno instead of a profile")
    ...
    if args.folded:
        n = write_folded(roots, args.out)
        print(f"\n{len(spans)} spans -> {n} folded stacks, {total:.2f} ms across "
              f"{len(roots)} block(s)")
        print(f"wrote {args.out}  —  render with:  inferno-flamegraph {args.out} > flame.svg")
        return
