...
use cf_utilities::migrations::{
	basics::HasVersion, v20100, v20200, v20300, AllVersions, ChangelogDetails, HasChangelog,
};
...
impl<T: HasChangelog> HasChangelog for hub::AssetMap<T> {
	...
	fn details() -> ChangelogDetails<Self> {
		// the problem is that we have to thread through the details here
		// somehow. All of the migrations in T could require details, but
		// we shouldn't be concerned about that!
		AllVersions::default()
	}
}
...
impl HasChangelog for Asset {
	...
	type in_20100 = _Asset::see_variant_changelogs_and_also<(
		(_Asset::variant::Wbtc::Added, _Asset::variant::ArbUsdt::Added),
		_Asset::variant::SolUsdt::Added,
	)>;
	type in_20200 = _Asset::see_variant_changelogs_and_also<(
		_Asset::variant::Trx::Added,
		_Asset::variant::TrxUsdt::Added,
	)>;
