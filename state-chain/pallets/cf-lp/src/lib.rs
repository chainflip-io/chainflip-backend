...
impl<T: Config> Pallet<T> {
	...
	pub fn mint_liquidity(
		...
		let mint_result = T::LiquidityPoolApi::mint(
			account_id.clone(),
			asset,
			range,
			liquidity_amount,
		...
		// Credit the user of any accrued fees
		Self::credit(&account_id, asset, mint_result.fees_returned[PoolSide::Asset0])?;
		Self::credit(
			&account_id,
			T::LiquidityPoolApi::STABLE_ASSET,
			mint_result.fees_returned[PoolSide::Asset1],
		)?;

		// Debit the user's asset from their account.
		Self::try_debit(&account_id, asset, mint_result.assets_spent[PoolSide::Asset0])?;
		Self::try_debit(
			&account_id,
			T::LiquidityPoolApi::STABLE_ASSET,
			mint_result.assets_spent[PoolSide::Asset1],
		)
	}
