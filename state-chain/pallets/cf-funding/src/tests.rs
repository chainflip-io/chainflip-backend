...
fn cannot_double_redeem() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Funding::inner_redeemed(ALICE, amount_a1));
		...
		assert_ok!(Funding::inner_redeemed(ALICE, amount_a2 - RedemptionTax::<Test>::get(),));
		assert!(PendingRedemptions::<Test>::get(&ALICE).is_none());
...
fn redemption_cannot_occur_without_funding_first() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Funding::inner_redeemed(BOB, FUNDING_AMOUNT),
			<Error<Test>>::NoPendingRedemption
		);

		// Valid Redeemed Event from Ethereum.
		assert_ok!(Funding::inner_redeemed(ALICE, REDEEMED_AMOUNT));
...
fn cannot_redeem_bond() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Funding::inner_redeemed(ALICE, AMOUNT - BOND));
		assert_noop!(
			Funding::redeem(
				RuntimeOrigin::signed(ALICE),
				1.into(),
...
fn redemption_expiry_removes_redemption() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Funding::inner_redeemed(ALICE, TOTAL_FUNDS),
			Error::<Test>::NoPendingRedemption
		);
...
fn restricted_funds_pay_redemption_tax() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Funding::inner_redeemed(ALICE, REDEEM_AMOUNT));
		assert_eq!(
			*RestrictedBalances::<Test>::get(ALICE).get(&RESTRICTED_ADDRESS).unwrap(),
...
fn vesting_contracts_test_case() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Funding::inner_redeemed(ALICE, 50));
		...
		assert_ok!(Funding::inner_redeemed(ALICE, 100));
		...
		assert_ok!(Funding::inner_redeemed(ALICE, 400));
	});
}
...
fn redeem_funds_until_restricted_balance_is_zero_and_then_redeem_to_redeem_address() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Funding::inner_redeemed(ALICE, AMOUNT));
...
fn can_redeem_if_balance_lower_than_restricted_funds() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Funding::inner_redeemed(ALICE, 80));
		assert_eq!(RestrictedBalances::<Test>::get(ALICE).get(&RESTRICTED_ADDRESS_1), None);
		assert_ok!(Funding::redeem(
			RuntimeOrigin::signed(ALICE),
			RedemptionAmount::Max,
...
fn account_references_must_be_zero_for_full_redeem() {
	...
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Funding::inner_redeemed(
			ALICE,
			FUNDING_AMOUNT,
		),);
