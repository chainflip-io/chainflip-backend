use crate::{
	mock::*, pallet, ActiveBidder, Error, EvmAddress, PendingRedemptions, RedemptionAmount,
...
fn restricted_funds_getting_recorded() {
	new_test_ext().execute_with(|| {
		const AMOUNT: u128 = 45;
		const RESTRICTED_ADDRESS: EvmAddress = [0xff; 20];
...
fn restricted_funds_getting_reduced() {
	new_test_ext().execute_with(|| {
		const RESTRICTED_ADDRESS: EvmAddress = [0x42; 20];
		const RESTRICTED_AMOUNT: FlipBalance = 50;
		const UNRESTRICTED_ADDRESS: EvmAddress = [0x01; 20];
...
fn vesting_contracts_test_case() {
	new_test_ext().execute_with(|| {
		// Contracts
		const VESTING_CONTRACT_1: EvmAddress = [0x01; 20];
		const VESTING_CONTRACT_2: EvmAddress = [0x02; 20];
		const UNRESTRICTED_ADDRESS: EvmAddress = [0x03; 20];
		// Balances
...
fn can_withdraw_unrestricted_to_restricted() {
	new_test_ext().execute_with(|| {
		// Contracts
		const RESTRICTED_ADDRESS_1: EvmAddress = [0x01; 20];
		const RESTRICTED_ADDRESS_2: EvmAddress = [0x02; 20];
		const UNRESTRICTED_ADDRESS: EvmAddress = [0x03; 20];
		// Balances
		const AMOUNT: u128 = 100;
...
fn can_only_redeem_funds_to_bound_address() {
	new_test_ext().execute_with(|| {
		const RESTRICTED_ADDRESS_1: EvmAddress = [0x01; 20];
		const BOUND_ADDRESS: EvmAddress = [0x02; 20];
		const UNRESTRICTED_ADDRESS: EvmAddress = [0x03; 20];
		const AMOUNT: u128 = 100;
		RestrictedAddresses::<Test>::insert(RESTRICTED_ADDRESS_1, ());
...
fn redeem_funds_until_restricted_balance_is_zero_and_then_redeem_to_redeem_address() {
	new_test_ext().execute_with(|| {
		const RESTRICTED_ADDRESS: EvmAddress = [0x01; 20];
		const REDEEM_ADDRESS: EvmAddress = [0x02; 20];
		const UNRESTRICTED_ADDRESS: EvmAddress = [0x03; 20];
		const AMOUNT: u128 = 100;
		RestrictedAddresses::<Test>::insert(RESTRICTED_ADDRESS, ());
...
mod test_restricted_balances {
	...
	fn run_test<E: Into<DispatchError>>(
		...
		bound_redeem_address: EvmAddress,
...
fn can_bind_redeem_address() {
	new_test_ext().execute_with(|| {
		const REDEEM_ADDRESS: EvmAddress = [0x01; 20];
...
fn max_redemption_is_net_exact_is_gross() {
	...
	const UNRESTRICTED_ADDRESS: EvmAddress = [0x01; 20];
	const RESTRICTED_ADDRESS: EvmAddress = [0x02; 20];

	#[track_caller]
	fn do_test(
		redemption_address: EvmAddress,
