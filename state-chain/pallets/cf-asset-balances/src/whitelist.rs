...
impl<AccountId: Ord + Clone> WithdrawalWhitelist<AccountId> {
	...
	///
	/// An `Allow` that would grow the active set beyond `max_entries` is a no-op — this bounds
...
mod tests {
	...
	fn enforcement_is_fail_safe_and_account_wide() {
		...
		w.apply_change(&allow(eth(1)), MAX_ENTRIES);
	...
	fn removing_last_address_keeps_blocking() {
		let mut w = Whitelist::default();
		w.apply_change(&allow(eth(1)), MAX_ENTRIES);
		w.apply_change(&remove(eth(1)), MAX_ENTRIES);
	...
	fn internal_account_allowlist_enforcement() {
		let mut w = Whitelist::default();
		w.apply_change(&allow_account(1), MAX_ENTRIES);
	...
	fn allow_is_dropped_at_entry_cap() {
		...
		let max_entries = 2;
		w.apply_change(&allow(eth(1)), max_entries);
		w.apply_change(&allow_account(7), max_entries);
		// At the cap: a further Allow is a no-op...
