...
fn test_buy_back_flip() {
	new_test_ext().execute_with(|| {
		...
		CollectedNetworkFee::<Test>::insert(STABLE_ASSET, NETWORK_FEE_AMOUNT);
		...
		assert_eq!(NETWORK_FEE_AMOUNT, CollectedNetworkFee::<Test>::get(STABLE_ASSET));
		...
		assert_eq!(0, CollectedNetworkFee::<Test>::get(STABLE_ASSET));
...
fn normal_swap_uses_correct_network_fee() {
	...
	// Btc will use the oracle to calculate the minimum fee. This means a small oracle slippage will be a added to the minimum.
	const MINIMUM_NETWORK_FEE_BTC: AssetAmount = (MINIMUM_NETWORK_FEE_USDC / BTC_PRICE_USD as AssetAmount) + 1;
		...
		.then_execute_with(|_| {
			assert_has_matching_event!(
				...
				RuntimeEvent::Swapping(Event::<Test>::SwapExecuted {
					network_fee,
					input: AssetAndAmount{ asset: Asset::Btc, amount: AMOUNT },
					..
				}) if *network_fee == AssetAndAmount { asset: Asset::Btc, amount: NETWORK_FEE_FOR_BTC * AMOUNT },
			);
			assert_has_matching_event!(
				Test,
				RuntimeEvent::Swapping(Event::<Test>::SwapExecuted {
					network_fee,
					input: AssetAndAmount{ asset: Asset::Flip, amount: AMOUNT },
					..
				}) if *network_fee == AssetAndAmount { asset: Asset::Flip, amount: NETWORK_FEE * AMOUNT },
			);
			assert_has_matching_event!(
				Test,
				RuntimeEvent::Swapping(Event::<Test>::SwapExecuted {
					network_fee,
					input: AssetAndAmount{ asset: Asset::Btc, amount: SMALL_AMOUNT },
					..
				}) if *network_fee == AssetAndAmount { asset: Asset::Btc, amount: MINIMUM_NETWORK_FEE_BTC },
			);
			assert_has_matching_event!(
				Test,
				RuntimeEvent::Swapping(Event::<Test>::SwapExecuted {
					network_fee,
					input: AssetAndAmount{ asset: Asset::Flip, amount: SMALL_AMOUNT },
					..
				}) if *network_fee == AssetAndAmount { asset: Asset::Flip, amount: MINIMUM_NETWORK_FEE_FLIP },
			);
		...
		.then_execute_with(|_| {
			...
			assert_has_matching_event!(
				...
				RuntimeEvent::Swapping(Event::<Test>::SwapExecuted {
					...
					input: AssetAndAmount { asset: Asset::Flip, amount: SMALL_AMOUNT },
					..
				}) if *network_fee == AssetAndAmount { asset: Asset::Flip, amount: MINIMUM_NETWORK_FEE },
			);
...
fn expect_earned_fees_to_be_recorded() {
	...
	const BOB_FEE_1: AssetAmount =
		(INPUT_AMOUNT - NETWORK_FEE_3) * BOB_FEE_BPS as u128 / 10_000;
	...
	new_test_ext()
		...
		.then_process_blocks(SWAP_DELAY_BLOCKS)
		.then_execute_with(|_| {
			const EXPECTED_OUTPUT_AMOUNT: AssetAmount = (INPUT_AMOUNT - NETWORK_FEE_1 - ALICE_FEE_1) * DEFAULT_SWAP_RATE ;
			assert_has_matching_event!(
				Test,
				RuntimeEvent::Swapping(Event::<Test>::SwapExecuted {
				...
				input: AssetAndAmount { asset: Asset::Flip, amount: INPUT_AMOUNT },
		...
		.then_process_blocks(SWAP_DELAY_BLOCKS)
		.then_execute_with(|_| {
			const EXPECTED_OUTPUT_AMOUNT: AssetAmount = (INPUT_AMOUNT - NETWORK_FEE_2 - ALICE_FEE_2) * DEFAULT_SWAP_RATE;
			assert_has_matching_event!(
				Test,
				RuntimeEvent::Swapping(Event::<Test>::SwapExecuted {
					...
					input: AssetAndAmount { asset: Asset::Usdc, amount: INPUT_AMOUNT },
		...
		.then_execute_with(|_| {
			...
			assert_has_matching_event!(
				...
				intermediate: Some(AssetAndAmount { asset: STABLE_ASSET, amount: INTERMEDIATE_AMOUNT }),
				..
			}));

			assert_eq!(
				get_broker_balance::<Test>(&ALICE, Asset::Flip),
				ALICE_FEE_1 + ALICE_FEE_3
			);
		...
		.then_execute_with(|_| {
			...
			assert_has_matching_event!(
				...
				RuntimeEvent::Swapping(Event::SwapExecuted {
					swap_request_id: SWAP_REQUEST_ID,
					swap_id: SwapId(1),
					input: AssetAndAmount { asset: Asset::Flip, amount: CHUNK_SIZE },
					network_fee,
					broker_fee,
					output,
					..
...
fn test_network_fee_tracking_when_rescheduled() {
	...
	const EXPECTED_BROKER_FEE: AssetAmount = (INPUT_AMOUNT - NETWORK_FEE_MINIMUM) * BROKER_FEE_BPS as u128 / 10_000;
	...
	new_test_ext()
		...
		.then_execute_with(|_| {
			...
			const EXPECTED_OUTPUT_AMOUNT: AssetAmount = (INPUT_AMOUNT - NETWORK_FEE_MINIMUM - EXPECTED_BROKER_FEE) * 4;
			assert_has_matching_event!(
				Test,
				RuntimeEvent::Swapping(Event::SwapExecuted {
					...
					output: AssetAndAmount { asset: OUTPUT_ASSET, amount: EXPECTED_OUTPUT_AMOUNT }, // (3584 - 3168)/4 = 104
					..
				})
			);
