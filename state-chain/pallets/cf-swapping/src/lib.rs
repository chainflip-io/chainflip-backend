...
impl<T: Config> SwapState<T> {
	...
	fn input_amount_before_fees(&self) -> AssetAmount {
		self.swap.input_amount + self.fees_amount()
	}

	fn fees_amount(&self) -> AssetAmount {
		self.network_fee_taken.unwrap_or_default() + self.broker_fee_taken.unwrap_or_default()
	}
	...
	pub fn intermediate_amount(&self) -> Option<AssetAmount> {
		if self.input_asset() == STABLE_ASSET || self.output_asset() == STABLE_ASSET {
			...
			self.intermediate.as_ref().map(|a| a.amount)
		}
	}
}
...
impl<T: Config> Swap<T> {
	...
	pub fn take_fee(&mut self, amount: AssetAmount) {
...
pub mod pallet {
	...
	pub enum Event<T: Config> {
		...
		SwapRequested {
			swap_request_id: SwapRequestId,
			input_asset: Asset,
			input_amount: AssetAmount, // Before network and broker fees
			output_asset: Asset,
		...
		SwapExecuted {
			swap_request_id: SwapRequestId,
			swap_id: SwapId,
			// Input amount excludes network and broker fees.
	...
	impl<T: Config> Pallet<T> {
		...
		fn take_broker_fees(
			...
			if total_fee_bps > ONE_AS_BASIS_POINTS {
				FeeTaken { remaining_amount: input_amount, fee: 0 }
			} else {
				let total_fee = broker_fees
					...
					.fold(0u128, |fee_accumulator, Beneficiary { account, bps }| {
						...
						T::BalanceApi::credit_account(account, input_asset, fee);
				...
				FeeTaken {
					remaining_amount: input_amount.saturating_sub(total_fee),
					fee: total_fee,
				}
			}
		}
		...
		fn swap_into_stable_taking_fees(
			...
			for swap in swaps.iter_mut() {
				// Get the fee tracker from the swap request
					...
					for fee_type in fees.iter_mut() {
						let fee = match fee_type {
							FeeType::NetworkFee(fee_tracker) => {
								let FeeTaken { fee, .. } =
									fee_tracker.take_fee(swap.input_amount());
							...
							FeeType::BrokerFee(beneficiaries) => {
								let FeeTaken { fee, .. } = Self::take_broker_fees(
									swap.input_amount(),
									swap.input_asset(),
									beneficiaries,
								);
						...
						swap.swap.take_fee(fee);
					}

					if swap.output_asset() == swap.input_asset() {
						swap.final_output = Some(swap.input_amount());
					}
					if swap.input_asset() == STABLE_ASSET {
						swap.intermediate = Some(AssetAndAmount {
							asset: STABLE_ASSET,
			...
			// Accrue the total network fee taken in storage
		...
		fn process_swap_outcome(swap: SwapState<T>) {
			...
			Self::deposit_event(Event::<T>::SwapExecuted {
				...
				input: AssetAndAmount::new(swap.input_asset(), swap.input_amount_before_fees()),
				...
				intermediate: swap
					.intermediate_amount()
					.map(|amount| AssetAndAmount::new(STABLE_ASSET, amount)),
		...
		pub fn get_network_fee_for_swap(
			...
			let (input_asset_fee, output_asset_fee, usd_minimum) = if is_internal_swap {
