...
fn expect_earned_fees_to_be_recorded() {
	...
	new_test_ext()
		...
		.then_execute_with(|_| {
			...
			assert_eq!(EarnedBrokerFees::<Test>::get(ALICE, Asset::Usdc), ALICE_FEE_1);
		})
		.execute_with(|| {
			swap_with_custom_broker_fee(
				Asset::Usdc,
		...
		.then_execute_with(|_| {
			...
			assert_eq!(
				EarnedBrokerFees::<Test>::get(ALICE, Asset::Usdc),
				ALICE_FEE_1 + ALICE_FEE_2
			);
		})
		.execute_with(|| {
			swap_with_custom_broker_fee(
		...
		.then_execute_with(|_| {
			...
			assert_eq!(EarnedBrokerFees::<Test>::get(BOB, Asset::Usdc), BOB_FEE_1);
		});
}
...
fn swap_broker_fee_calculated_correctly() {
	...
	new_test_ext()
		...
		.then_execute_at_block(3u32, |_| {})
		.then_execute_with(|_| {
			assert_eq!(EarnedBrokerFees::<Test>::get(ALICE, Asset::Usdc), total_fees);
		});
}
