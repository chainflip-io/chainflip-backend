...
impl<T: Config> ChainVault for EthereumChain<T> {
	...
	type Error = RotationError<T::AccountId>;
...
impl<T: Config>
	RequestResponse<
		...
		ThresholdSignatureResponse<T::AccountId, SchnorrSignature>,
		RotationError<T::AccountId>,
	> for EthereumChain<T>
{
	/// Make the request to sign by emitting an event
	fn make_request(
		ceremony_id: CeremonyId,
		request: ThresholdSignatureRequest<T::PublicKey, T::AccountId>,
	) -> Result<(), RotationError<T::AccountId>> {
	...
	fn handle_response(
		ceremony_id: CeremonyId,
		response: ThresholdSignatureResponse<T::AccountId, SchnorrSignature>,
	) -> Result<(), RotationError<T::AccountId>> {
		match response {
