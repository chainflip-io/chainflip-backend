...
fn test_should_mint_at() {
	new_test_ext(vec![], None, None).execute_with(|| {
		...
		assert_eq!(Pallet::<Test>::should_mint_at(MINT_INTERVAL), true);
		// It hasn't yet been `MINT_INTERVAL` blocks since the last mint.
		assert_eq!(Pallet::<Test>::should_mint_at(MINT_INTERVAL - 1), false);
		// It has been more than `MINT_INTERVAL` blocks since the last mint.
		assert_eq!(Pallet::<Test>::should_mint_at(MINT_INTERVAL + 1), true);
		// We have literally *just* minted.
		assert_eq!(Pallet::<Test>::should_mint_at(0), false);
	});
}
