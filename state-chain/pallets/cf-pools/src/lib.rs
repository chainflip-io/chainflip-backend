...
use cf_primitives::{
	...
	BurnResult, Liquidity, MintResult, MintedLiquidity, PoolAssetMap, Tick,
};
	...
	fn mint(
		...
		Pools::<T>::try_mutate(asset, |maybe_pool| {
			if let Some(pool) = maybe_pool.as_mut() {
				...
				let (assets_spent_u256, fees_returned) = pool
				...
				Ok(MintResult::new(assets_debited, fees_returned))
			} else {
