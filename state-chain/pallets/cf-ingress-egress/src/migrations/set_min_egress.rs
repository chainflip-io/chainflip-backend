...
impl<T: Config<Instance3, TargetChain = Bitcoin>> OnRuntimeUpgrade for Migration<T, Instance3> {
	fn on_runtime_upgrade() -> frame_support::weights::Weight {
		MinimumEgress::<T, Instance3>::set(btc::Asset::Btc, BITCOIN_DUST_LIMIT);
	...
	fn pre_upgrade() -> Result<Vec<u8>, DispatchError> {
		assert_eq!(MinimumEgress::<T, Instance3>::get(btc::Asset::Btc), 0);
		Ok(vec![])
	}

	#[cfg(feature = "try-runtime")]
	fn post_upgrade(_state: Vec<u8>) -> Result<(), DispatchError> {
		assert_eq!(MinimumEgress::<T, Instance3>::get(btc::Asset::Btc), BITCOIN_DUST_LIMIT);
