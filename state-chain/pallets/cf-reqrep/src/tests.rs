use crate::{self as pallet_cf_request_response, Error, PendingRequests, mock::*};
...
fn ping_pong() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			PingPongRequestResponse::response(RawOrigin::Signed(0).into(), request_id + 1, ping_pong::Pong),
