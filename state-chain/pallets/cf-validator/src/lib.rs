    ...
    impl<T: Config> Pallet<T> {
        ...
        pub(super) fn set_validator_target_size(
            ...
            ensure_root(origin)?;
                ...
                size >= T::MinValidatorSetSize::get().saturated_into(),
                Error::<T>::InvalidValidatorSetSize
            );
...
impl<T: Config> pallet_session::SessionHandler<T::ValidatorId> for Pallet<T> {
    ...
    fn on_new_session<Ks: OpaqueKeys>(
        ...
        let next_validators = queued_validators
            .iter()
            .map(|(id, _)| id.clone())
            .collect::<Vec<T::ValidatorId>>();
    ...
    pub fn run_auction(index: EpochIndex) -> Option<Vec<T::ValidatorId>> {
        ...
        if !candidates.is_empty() {
            ...
            if let Some(candidates) = candidates {
                let candidates: Vec<T::ValidatorId> =
                    candidates.iter().map(|i| i.0.clone()).collect();
