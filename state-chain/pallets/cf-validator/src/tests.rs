	...
	new_test_ext().execute_with(|| {
		...
		impl TestMintTracker {
			fn get_minted() -> BTreeMap<u64, u128> {
				frame_support::storage::unhashed::get(b"test_minted")
					.unwrap_or_default()
			}
		...
		let snapshot = DelegationSnapshot::<Test> {
			...
			delegators: [
				(DELEGATOR1, 2000000u128),
				(DELEGATOR2, 3000000u128),
			].into_iter().collect(),
			delegation_fee_bps: 500, // 5% fee
		};
		
		// Register the snapshot for the current epoch
		DelegationResolver::<Test>::register_snapshot(5, snapshot);
