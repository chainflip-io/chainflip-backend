...
pub struct WbtcMigration<T>(PhantomData<T>);

impl<T: Config<Hash = H256>> UncheckedOnRuntimeUpgrade for WbtcMigration<T> {
	fn on_runtime_upgrade() -> Weight {
		log::info!("🌮 Running migration for Environment pallet: Adding WBTC to EthereumSupportedAssets ...");

		// TODO ADD deployed addresses for testnet and devnet
