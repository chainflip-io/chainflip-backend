...
pub mod pallet {
	...
	pub trait Config: cf_traits::Chainflip {
		...
		type CloseSolanaVaultSwapAccounts: cf_chains::CloseSolanaVaultSwapAccounts;

		type SolanaBroadcaster: Broadcaster<Solana, ApiCall = Self::CloseSolanaVaultSwapAccounts>;
		...
		fn on_idle(block_number: BlockNumberFor<T>, _remaining_weight: Weight) -> Weight {
			if Self::get_number_of_available_sol_nonce_accounts() > NONCE_AVAILABILITY_THRESHOLD_FOR_INITIATING_SWAP_ACCOUNT_CLOSURES &&
				...
				SolanaOpenContractSwapAccounts::<T>::try_mutate(|accounts| {
					...
					match T::CloseSolanaVaultSwapAccounts::new_unsigned(accounts_to_close.clone()) {
						Ok(apicall) => {
							let (broadcast_id, _) =
