...
pub mod pallet {
	...
	/// Configuration that is used on a CompositeElectoralSystem level, used to establish
	/// interaction with the rest of the runtime via:
	///  - SafeMode
	///  - Events
	///  - Hook for calling into electoral systems via governance
	pub trait ElectoralSystemConfiguration {
	...
	pub trait Config<I: 'static = ()>: Chainflip {
		...
		type ElectoralSystemConfiguration: ElectoralSystemConfiguration;

		/// The weights for the pallet
		type WeightInfo: WeightInfo;

		type SafeMode: Get<
			<Self::ElectoralSystemConfiguration as ElectoralSystemConfiguration>::SafeMode,
		>;
	}
	...
	pub enum Event<T: Config<I>, I: 'static = ()> {
		...
		ElectoralEvent(
			<T::ElectoralSystemConfiguration as ElectoralSystemConfiguration>::ElectoralEvents,
		),
	}
	...
	impl<T: Config<I>, I: 'static> Pallet<T, I> {
		...
		// Call indices 2, 3 and 4 previously held `ignore_my_votes`, `stop_ignoring_my_votes`
		// and `delete_vote`, and are deliberately left unused. A validator has no need to retract
		// a vote: `vote` replaces an existing vote in place, and withdrawing a vote is
		// indistinguishable from holding a wrong one as far as threshold consensus is concerned.
		// When an election's premise becomes void it is the electoral system that drops every
		// validator's votes, via `clear_election_votes`/`delete_election`.
		...
		pub fn start_new_block_witnesser_election(
			origin: OriginFor<T>,
			properties: <T::ElectoralSystemConfiguration as ElectoralSystemConfiguration>::Properties,
		) -> DispatchResult {
			T::EnsureGovernance::ensure_origin(origin)?;

			T::ElectoralSystemConfiguration::start(properties);
