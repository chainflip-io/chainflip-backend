...
// // 3. On finalize we start as many elections as possible within the new range, but no more than
// the //    maximum => skipping received blocks is fine, even ranges
...
// // 5. Ranges are consistent, and if we're mid range we don't emit a new election, we wait for
// next //    time we can election for a whole range.
// // 6. State updates. When channel is opened, what happens? new election?
// // Out of order consensus - when catching up this is possible. We need to ensure everything is
// still // handled correctly.
// // Testing with a chain with range > 1
// // State partially processed, how do we test that the state still gets processed until all the
// state // is processed.
//
// // use super::{
...
//
// thread_local! {
// 	pub static PROPERTIES_TO_RETURN: std::cell::RefCell<Properties> = const {
// std::cell::RefCell::new(BTreeSet::new()) }; 	pub static GENERATE_ELECTION_HOOK_CALLED:
// std::cell::Cell<u8> = const { std::cell::Cell::new(0) }; 	pub static
// PROCESS_BLOCK_DATA_HOOK_CALLED: std::cell::Cell<u8> = const { std::cell::Cell::new(0) }; 	// the
// actual block data that process_block_data was called with. 	pub static
// PROCESS_BLOCK_DATA_CALLED_WITH: std::cell::RefCell<Vec<(ChainBlockNumber, BlockData)>> = const {
// std::cell::RefCell::new(vec![]) }; 	// Flag to pass through block data:
// 	pub static PASS_THROUGH_BLOCK_DATA: std::cell::Cell<bool> = const { std::cell::Cell::new(true)
// }; 	pub static PROCESS_BLOCK_DATA_TO_RETURN: std::cell::RefCell<Vec<(ChainBlockNumber,
// BlockData)>> = const { std::cell::RefCell::new(vec![]) }; }
//
...
// 			assert_eq!(GENERATE_ELECTION_HOOK_CALLED.with(|hook_called| hook_called.get()), n,
// "generate_election_properties should have been called {} times so far!", n); 		},
// 		number_of_open_elections_is(_pre, post, n: ElectionCount) {
// 			assert_eq!(post.unsynchronised_state.elections_open_for.len(), n as usize, "Number of open
