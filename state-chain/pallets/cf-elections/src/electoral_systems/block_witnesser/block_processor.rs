...
pub(crate) mod tests {
	...
	fn already_processed_events_always_have_highest_expiry_block_number() {
		...
		assert_eq!(
			...
			Some(10u8.saturating_add((SAFETY_MARGIN * 2) as u8)).as_ref()
		);
	}
	...
	fn same_block_twice() {
		let mut processor = BlockProcessor::<Types>::default();
		// We processed pre-witnessing (boost) for the followings deposit
		processor.process_block_data(ChainProgressInner::Progress(9), Some((9, vec![1], 3)));
		processor.process_block_data(ChainProgressInner::Progress(10), Some((10, vec![1], 3)));
	}
	...
	pub fn generate_state() -> impl Strategy<Value = BlockProcessor<Types>> {
		...
		let block_info = (block_data, 0..10u32, 0..5u32).prop_map(|(b, age, margin)| {
			BlockProcessingInfo { block_data: b, next_age_to_process: age, safety_margin: margin }
		});
		(
			proptest::collection::btree_map(any::<BlockNumber>(), block_info, 0..10),
			proptest::collection::btree_map(any::<MockBtcEvent>(), any::<BlockNumber>(), 0..10),
		)
			.prop_map(|(blocks_data, processed_events)| BlockProcessor {
				blocks_data,
				processed_events,
				..Default::default()
			})
	...
	pub fn test_block_processor() {
		...
		// TODO: make block processor work for safety margin 0!
		let safety_margin = 1..10u32;
