//! Autogenerated weights for pallet_cf_online
//!
//! THIS FILE WAS AUTO-GENERATED USING THE SUBSTRATE BENCHMARK CLI VERSION 3.0.0
//! DATE: 2021-11-05, STEPS: `[20, ]`, REPEAT: 10, LOW RANGE: `[]`, HIGH RANGE: `[]`
//! EXECUTION: Some(Wasm), WASM-EXECUTION: Interpreted, CHAIN: None, DB CACHE: 128

// Executed Command:
// ./target/release/state-chain-node
// benchmark
// --extrinsic
// *
// --pallet
// pallet_cf_online
// --output
// state-chain/pallets/cf-online/src/weights.rs
// --execution=wasm
// --steps=20
// --repeat=10
// --template=state-chain/chainflip-weight-template.hbs


#![cfg_attr(rustfmt, rustfmt_skip)]
#![allow(unused_parens)]
#![allow(unused_imports)]

use frame_support::{traits::Get, weights::{Weight, constants::RocksDbWeight}};
use sp_std::marker::PhantomData;

/// Weight functions needed for pallet_cf_online.
pub trait WeightInfo {
	fn heartbeat() -> Weight;
	fn submit_network_state() -> Weight;
	fn on_initialize_no_action() -> Weight;
}

/// Weights for pallet_cf_online using the Substrate node and recommended hardware.
pub struct PalletWeight<T>(PhantomData<T>);
impl<T: frame_system::Config> WeightInfo for PalletWeight<T> {
	fn heartbeat() -> Weight {
		(91_000_000 as Weight)
			.saturating_add(T::DbWeight::get().reads(2 as Weight))
			.saturating_add(T::DbWeight::get().writes(2 as Weight))
	}
	fn submit_network_state() -> Weight {
		(8_724_000_000 as Weight)
			.saturating_add(T::DbWeight::get().reads(304 as Weight))
			.saturating_add(T::DbWeight::get().writes(151 as Weight))
	}
	fn on_initialize_no_action() -> Weight {
		(2_000_000 as Weight)
	}
}

// For backwards compatibility and tests
impl WeightInfo for () {
	fn heartbeat() -> Weight {
		(91_000_000 as Weight)
			.saturating_add(RocksDbWeight::get().reads(2 as Weight))
			.saturating_add(RocksDbWeight::get().writes(2 as Weight))
	}
	fn submit_network_state() -> Weight {
		(8_724_000_000 as Weight)
			.saturating_add(RocksDbWeight::get().reads(304 as Weight))
			.saturating_add(RocksDbWeight::get().writes(151 as Weight))
	}
	fn on_initialize_no_action() -> Weight {
		(2_000_000 as Weight)
	}
}