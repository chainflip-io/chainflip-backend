...
pub mod pallet {
	...
	/// Members that an open proposal would add to the voting authority. They all have to approve
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn propose_governance_extrinsic(
			...
			let incoming = Self::incoming_members(&authority, call.as_ref());
		...
		pub fn approve(
			...
			let authority = Members::<T>::get();
				...
				authority.is_member(&account_id) ||
	...
	impl<T: Config> BuildGenesisConfig for GenesisConfig<T> {
		fn build(&self) {
			let authority = VotingAuthority::simple_group(
			...
			if !self.members.is_empty() {
				authority.validate().expect("The genesis voting authority must be valid");
			}
			for member in &self.members {
				<frame_system::Pallet<T>>::inc_sufficients(member);
			}
			Members::<T>::set(authority);
...
impl<T: Config> Pallet<T> {
	/// The members `call` would add to the authority, if it is a `set_voting_authority` call
	/// carrying a valid authority. An invalid one can never be installed, and ignoring it keeps
	/// this bounded by `MAX_MEMBERS`.
	fn incoming_members(
		authority: &VotingAuthority<T::AccountId>,
		call: &<T as Config>::RuntimeCall,
	) -> BTreeSet<T::AccountId> {
		match call.is_sub_type() {
			Some(Call::set_voting_authority { new_authority })
				if new_authority.validate().is_ok() =>
				new_authority.members().difference(&authority.members()).cloned().collect(),
			_ => Default::default(),
		}
	}
