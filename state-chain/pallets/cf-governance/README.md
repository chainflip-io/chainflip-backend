...
Governance members are organised as a Council: a tree of at most three levels whose leaves are individual accounts, grouped into Simple Groups (k-of-n members) or Weighted Groups (members carry weights and the group passes once the weight of its passing members reaches a threshold). Members approve proposals individually; a proposal executes as soon as the approvals satisfy the root of the tree. The council is replaced via the governance-gated `set_council` extrinsic, which rejects any structure whose threshold could never be reached.
...
- Council: the (possibly nested and weighted) structure of groups and individuals whose approvals decide a Proposal.
- Governance Quorum: the approvals that satisfy the root of the Council.
