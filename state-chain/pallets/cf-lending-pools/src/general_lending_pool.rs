...
impl<T: Config> LendingPool<T> {
	...
	pub fn remove_funds(
		...
		let required_amount =
			core::cmp::min(*share * self.total_amount, amount.unwrap_or(u128::MAX));

		let amount_to_withdraw = required_amount.min(self.available_amount);
		...
		let remaining_owed_amount = required_amount.saturating_sub(amount_to_withdraw);
...
mod tests {
	...
	fn adding_and_removing_funds() {
		...
		assert_eq!(chp_pool.remove_funds(&LENDER_2), 300);
		assert_eq!(chp_pool.total_amount, 600);
		assert_eq!(chp_pool.available_amount, 600);
		...
		assert_eq!(chp_pool.remove_funds(&LENDER_3), 300);
		assert_eq!(chp_pool.total_amount, 300);
		assert_eq!(chp_pool.available_amount, 300);

		check_shares(&chp_pool, [(LENDER_1, Perquintill::from_percent(100))]);

		assert_eq!(chp_pool.remove_funds(&LENDER_1), 300);
	...
	fn remove_funds_partially() {
		...
		// Lender 1 gets only part of their funds:
		assert_eq!(chp_pool.remove_funds(&LENDER_1), 400);
