...
fn lender_basic_adding_and_removing_funds() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(LendingPools::remove_lender_funds(RuntimeOrigin::signed(LENDER), LOAN_ASSET));
		...
		System::assert_has_event(RuntimeEvent::LendingPools(Event::<Test>::LendingFundsRemoved {
			lender_id: LENDER,
			asset: LOAN_ASSET,
			unlocked_amount: INIT_POOL_AMOUNT,
		}));
	});
}
...
fn basic_loan_aggregation() {
	...
	new_test_ext().execute_with(|| {
		...
		assert_err!(
			LendingPools::expand_loan(BORROWER, LOAN_ID, EXTRA_PRINCIPAL_2, Default::default()),
...
fn safe_mode_for_removing_funds() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			LendingPools::remove_lender_funds(RuntimeOrigin::signed(LENDER), LOAN_ASSET),
