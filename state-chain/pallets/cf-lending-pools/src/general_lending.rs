...
impl<T: Config> OraclePriceCache<T> {
	pub fn new() -> Self {
		OraclePriceCache { cached_prices: Default::default(), _phantom: PhantomData }
	}

	fn get_price(&self, asset: Asset) -> Result<Price, Error<T>> {
		...
		let cached_price = match self.cached_prices.borrow_mut().entry(asset) {
			Entry::Vacant(entry) => {
				...
				if let Some(valid_price) = T::PriceApi::get_price(asset).and_then(|oracle_price| {
					...
					// TODO: check if stale (currently this breaks benchmarks)
	...
	fn remove_collateral(
		...
		LoanAccounts::<T>::mutate(borrower_id, |maybe_account| {
			...
			if !loan_account.loans.is_empty() &&
				loan_account.derive_ltv(&mut price_cache)? >
					chp_config.ltv_thresholds.target.into()
			{
