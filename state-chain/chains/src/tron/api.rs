...
/// Chainflip api calls available on Tron.
#[derive(CloneNoBound, DebugNoBound, PartialEqNoBound, EqNoBound, Encode, Decode, TypeInfo)]
#[scale_info(skip_type_params(Environment))]
pub enum TronApi<Environment: 'static> {
...
impl<E> SetAggKeyWithAggKey<EvmCrypto> for TronApi<E>
where
...
impl<E> AllBatch<Tron> for TronApi<E>
where
	E: EvmEnvironmentProvider<Tron> + ReplayProtectionProvider<Tron>,
{
	fn new_unsigned_impl(
...
impl<E> ExecutexSwapAndCall<Tron> for TronApi<E>
where
	E: EvmEnvironmentProvider<Tron> + ReplayProtectionProvider<Tron>,
{
	fn new_unsigned_impl(
		transfer_param: TransferAssetParams<Tron>,
...
impl<E> TransferFallback<Tron> for TronApi<E>
where
	E: EvmEnvironmentProvider<Tron> + ReplayProtectionProvider<Tron>,
{
	fn new_unsigned_impl(
...
impl<E> ConsolidateCall<Tron> for TronApi<E>
where
	E: EvmEnvironmentProvider<Tron> + ReplayProtectionProvider<Tron>,
{
	...
	E: EvmEnvironmentProvider<Tron> + ReplayProtectionProvider<Tron>,
{
	fn new_unsigned(
		...
		transfer: TransferForRejection<Tron>,
	) -> Result<Self, RejectError> {
		use FetchForRejection::*;
		use TransferForRejection::*;

		match evm_all_batch_builder::<Tron, _>(
			match fetch {
...
macro_rules! map_over_api_variants {
	( $self:expr, $var:pat_param, $var_method:expr $(,)* ) => {
		match $self {
			TronApi::SetAggKeyWithAggKey($var) => $var_method,
			TronApi::AllBatch($var) => $var_method,
			TronApi::ExecutexSwapAndCall($var) => $var_method,
			TronApi::TransferFallback($var) => $var_method,
			TronApi::RejectCall($var) => $var_method,
			TronApi::_Phantom(..) => unreachable!(),
		}
	};
}
...
impl<E: ReplayProtectionProvider<Tron> + EvmEnvironmentProvider<Tron>> ApiCall<EvmCrypto>
	for TronApi<E>
{
