...
pub mod polkadot_sdk_types {
	...
	impl<Call, Extra: SignedExtension> GenericUncheckedExtrinsic<Call, Extra> {
		pub fn signature(&self) -> Option<MultiSignature> {
			self.0.preamble.clone().to_signed().map(|(_, sig, _)| sig)
		}
	}

	/// Type to represent the version of the [Extension](TransactionExtension) used in this
	/// extrinsic.
	...
	/// compatibility reasons. It will be deprecated in favor of v5 extrinsics and an
	/// inherent/general transaction model.
	pub const LEGACY_EXTRINSIC_FORMAT_VERSION: ExtrinsicVersion = 4;

	/// A "header" for extrinsics leading up to the call itself. Determines the type of extrinsic
	/// and holds any necessary specialized data.
	#[derive(Eq, PartialEq, Clone)]
	pub enum Preamble<Address, Signature, Extension> {
		/// An extrinsic without a signature or any extension. This means it's either an inherent
		/// or an old-school "Unsigned" (we don't use that terminology any more since it's
		/// confusable with the general transaction which is without a signature but does have an
		/// extension).
		///
		...
		/// An old-school transaction extrinsic which includes a signature of some hard-coded
		/// crypto. Available only on extrinsic version 4.
		Signed(Address, Signature, Extension),
		/// A new-school transaction extrinsic which does not include a signature by default. The
		/// origin authorization, through signatures or other means, is performed by the
		/// transaction extension in this extrinsic. Available starting with extrinsic version 5.
	...
	impl<Address, Signature, Extension> core::fmt::Debug for Preamble<Address, Signature, Extension>
	where
		Address: core::fmt::Debug,
		Extension: core::fmt::Debug,
	{
		fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
			match self {
				Self::Bare(_) => write!(f, "Bare"),
				Self::Signed(address, _, tx_ext) =>
					write!(f, "Signed({:?}, {:?})", address, tx_ext),
	...
	/// An extrinsic right from the external world. This is unchecked and so can contain a
	/// signature.
	///
	/// An extrinsic is formally described as any external data that is originating from the outside
	/// of the runtime and fed into the runtime as a part of the block-body.
	///
	/// Inherents are special types of extrinsics that are placed into the block by the
	/// block-builder. They are unsigned because the assertion is that they are "inherently true"
	/// by virtue of getting past all validators.
	///
	...
	/// payment, but it could in principle be any other interaction. Transactions are either signed
	/// or unsigned. A sensible transaction pool should ensure that only transactions that are
	/// worthwhile are considered for block-building.
	///
	...
	pub struct UncheckedExtrinsic<Address, Call, Signature, Extension> {
		/// Information regarding the type of extrinsic this is (inherent or transaction) as well
		/// as associated extension (`Extension`) data if it's a transaction and a possible
		/// signature.
		...
		Extension: Decode,
	{
		fn decode<I: Input>(input: &mut I) -> Result<Self, codec::Error> {
			...
			// compatible with SCALE's generic `Vec<u8>` type. Basically this just means
			// accepting that there will be a prefix of vector length.
