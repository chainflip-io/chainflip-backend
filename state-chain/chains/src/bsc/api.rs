...
impl<E> SetAggKeyWithAggKey<EvmCrypto> for BscApi<E>
where
...
impl<E> AllBatch<Bsc> for BscApi<E>
where
	E: EvmEnvironmentProvider<Bsc> + ReplayProtectionProvider<Bsc>,
{
	fn new_unsigned_impl(
		fetch_params: Vec<FetchAssetParams<Bsc>>,
		transfer_params: Vec<(TransferAssetParams<Bsc>, EgressId)>,
...
impl<E> ExecutexSwapAndCall<Bsc> for BscApi<E>
where
	E: EvmEnvironmentProvider<Bsc> + ReplayProtectionProvider<Bsc>,
{
	fn new_unsigned_impl(
		transfer_param: TransferAssetParams<Bsc>,
...
impl<E> TransferFallback<Bsc> for BscApi<E>
where
	E: EvmEnvironmentProvider<Bsc> + ReplayProtectionProvider<Bsc>,
{
	fn new_unsigned_impl(
...
impl<E> ConsolidateCall<Bsc> for BscApi<E>
where
	E: EvmEnvironmentProvider<Bsc> + ReplayProtectionProvider<Bsc>,
{
	...
	E: EvmEnvironmentProvider<Bsc> + ReplayProtectionProvider<Bsc>,
{
	fn new_unsigned(
		...
		transfer: TransferForRejection<Bsc>,
	) -> Result<Self, RejectError> {
		use FetchForRejection::*;
		use TransferForRejection::*;

		match evm_all_batch_builder::<Bsc, _>(
			match fetch {
...
macro_rules! map_over_api_variants {
	( $self:expr, $var:pat_param, $var_method:expr $(,)* ) => {
		match $self {
			BscApi::SetAggKeyWithAggKey($var) => $var_method,
			BscApi::AllBatch($var) => $var_method,
			BscApi::ExecutexSwapAndCall($var) => $var_method,
			BscApi::TransferFallback($var) => $var_method,
			BscApi::RejectCall($var) => $var_method,
			BscApi::_Phantom(..) => unreachable!(),
		}
	};
}
...
impl<E: ReplayProtectionProvider<Bsc> + EvmEnvironmentProvider<Bsc>> ApiCall<EvmCrypto>
	for BscApi<E>
{
