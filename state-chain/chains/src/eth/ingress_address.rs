...
pub fn get_create_2_address(asset: &str, vault_address: [u8; 20], intent_id: u32) -> [u8; 20] {
	...
	// println!("create_2_args: {:?}", hex::encode(&create_2_args));
...
fn get_deploy_bytecode(asset: &str) -> Vec<u8> {
	match asset {
		"ETH:ETH" => DEPLOY_BYTECODE_ETH.to_vec(),
		"ETH:USDC" => DEPLOY_BYTECODE_TOKEN.to_vec(),
		"ETH:FLIP" => DEPLOY_BYTECODE_TOKEN.to_vec(),
		_ => unreachable!(),
	}
}
...
fn get_constructor_argument_bytes(asset: &str) -> Vec<u8> {
	match asset {
		"ETH:ETH" => vec![],
		// @FIXME replace with the USDC ADDRESS
		"ETH:USDC" => hex::decode("USDC_ADDRESS").unwrap(),
		// @FIXME replace with the actual FLIP ADDRESS
		"ETH:FLIP" => hex::decode("FLIP_ADDRESS").unwrap(),
		&_ => vec![],
	}
}
...
fn test_eth_eth() {
	...
	assert_eq!(
		get_create_2_address("ETH:ETH", VAULT_ADDRESS, 420696969),
