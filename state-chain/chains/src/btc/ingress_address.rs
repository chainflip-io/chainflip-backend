...
pub fn derive_btc_ingress_bitcoin_script(pubkey_x: [u8; 32], salt: u32) -> BitcoinScript {
	BitcoinScript { data: tweaked_pubkey(pubkey_x, salt).serialize_compressed()[1..33].to_vec() }
}

// Is this a general way to derive an address from a script or specific to our ingress scripts?
