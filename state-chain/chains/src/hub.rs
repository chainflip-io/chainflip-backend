...
pub const REFERENCE_HUBDOT_PRICE_IN_USD: PolkadotBalance = 4_000_000u128; //4.0 usd
	...
	fn estimate_fee(
		...
		match ingress_or_egress {
			...
			IngressOrEgress::IngressVaultSwap => {
				log_or_panic!("Tried to get IngressVaultSwap fees for Assethub, but vault swaps are not supported.");
				0
			},
			...
			IngressOrEgress::EgressCcm { gas_budget: _, message_length } => {
				// Estimated on Assethub
				const ESTIMATED_FEE_PER_BYTE: u128 = 200 * fee_constants::MICRO_DOT;
				let payload_len_fee: u128 =
					ESTIMATED_FEE_PER_BYTE.saturating_mul(message_length as u128);
				const CCM_CORE_FEE: u128 = 10 * fee_constants::MILLI_DOT;
				self.median_tip + CCM_CORE_FEE + payload_len_fee
			},
		}
	}
}
