...
use sp_std::{boxed::Box, fmt, iter::Peekable, str::CharIndices, vec, vec::Vec};
...
impl sp_std::fmt::Display for LexerError {
	fn fmt(&self, f: &mut sp_std::fmt::Formatter<'_>) -> sp_std::fmt::Result {
		match self {
			LexerError::UnrecognisedToken(u1, u2, s) =>
				write!(f, "UnrecognisedToken {}:{} `{}`", u1, u2, s),
			LexerError::ExpectedToken(u1, u2, s) =>
				write!(f, "Expected token `{}` at {}:{} ", s, u1, u2),
			LexerError::EndofFileInHex(u1, u2) => write!(f, "EndofFileInHex {}:{}", u1, u2),
			LexerError::MissingNumber(u1, u2) => write!(f, "MissingNumber {}:{}", u1, u2),
			LexerError::EndOfFileExpectedToken(s) => write!(f, "end of file but expected `{}`", s),
			LexerError::EndOfFile => write!(f, "end of file"),
		}
	}
}
