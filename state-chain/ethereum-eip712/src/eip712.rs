...
// Portions Copyright (c) 2020 Georgios Konstantopoulos
//
...
impl sp_std::fmt::Display for Eip712Error {
	fn fmt(&self, f: &mut sp_std::fmt::Formatter<'_>) -> sp_std::fmt::Result {
		match self {
			Eip712Error::SerdeJsonError(_) => write!(f, "Failed to serialize serde JSON object"),
			Eip712Error::FromHexError(_) => write!(f, "Failed to decode hex value"),
			Eip712Error::FailedToEncodeStruct =>
				write!(f, "Failed to make struct hash from values"),
			Eip712Error::TryFromSliceError(_) =>
				write!(f, "Failed to convert slice into byte array"),
			Eip712Error::NestedEip712StructNotImplemented =>
				write!(f, "Nested Eip712 struct not implemented. Failed to parse."),
			Eip712Error::Message(m) => write!(f, "Error from Eip712 struct: {:?}", m),
		}
	}
}

impl From<serde_json::Error> for Eip712Error {
	fn from(value: serde_json::Error) -> Self {
		Eip712Error::SerdeJsonError(value)
	}
}
impl From<hex::FromHexError> for Eip712Error {
	fn from(value: hex::FromHexError) -> Self {
		Eip712Error::FromHexError(value)
	}
}
impl From<core::array::TryFromSliceError> for Eip712Error {
	fn from(value: core::array::TryFromSliceError) -> Self {
		Eip712Error::TryFromSliceError(value)
	}
}
...
pub trait Eip712 {
	...
	type Error: sp_std::fmt::Display + Send + Sync + sp_std::fmt::Debug;
...
pub struct EIP712Domain {
	...
	#[serde(
		...
		deserialize_with = "crate::serde_helpers::deserialize_stringified_numeric_opt"
	)]
