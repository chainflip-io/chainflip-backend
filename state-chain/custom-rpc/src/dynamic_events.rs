...
use sc_client_api::{Backend, BlockchainEvents, HeaderBackend, StateBackend};
...
impl<C, B, S> DynamicApiServer for CustomRpc<C, B, S>
where
	B: BlockT<Hash = state_chain_runtime::Hash, Header = state_chain_runtime::Header>,
	C: sp_api::ProvideRuntimeApi<B>
		...
		+ BlockchainEvents<B>,
	C::Api: sp_api::Metadata<B> + sp_api::Core<B>,
	S: Backend<B> + Send + Sync + 'static,
	<S as Backend<B>>::State: StateBackend<BlakeTwo256>,
{
	fn cf_dynamic_events(
		&self,
		at: Option<state_chain_runtime::Hash>,
	) -> RpcResult<Vec<DynamicEventRecord>> {
		fn from_displayable_error(e: impl core::fmt::Display) -> jsonrpsee::core::Error {
			CallError::Failed(anyhow::anyhow!("{e}")).into()
		}
		...
		let events_data = self
			.state_backend
			.state_at(hash)
			.map_err(from_displayable_error)?
			.storage(&SYSTEM_EVENTS_STORAGE_KEY[..])
			.map_err(from_displayable_error)?
			.unwrap_or_default();
