...
// #[test]
// fn reward_distribution_estimate_reflects_projected_split() {
// 	use state_chain_runtime::chainflip::reward_distribution_estimate;

// 	const EPOCH_DURATION_BLOCKS: u32 = 200;
// 	const MAX_AUTHORITIES: AuthorityCount = 3;
// 	const RESERVE_AMOUNT: FlipBalance = 1_000_000;
// 	let managed_validator: AccountId = AccountId::from([0xcf; 32]);
...
// 			// Before FLIP 2.1 activates for this epoch, the estimate should be empty.
// 			let pre_activation = reward_distribution_estimate();
// 			assert!(pre_activation.operators.is_empty());
// 			assert!(pre_activation.solo_validators.is_empty());
// 			assert_eq!(pre_activation.pool_onchain_amount, 0);
// 			assert_eq!(pre_activation.per_authority_share, 0);

// 			// Activate FLIP 2.1 for the current epoch and seed the fee reserve.
// 			let epoch_index = Validator::current_epoch();
// 			pallet_cf_flip::FeeRewardsActivationEpoch::<Runtime>::set(epoch_index);
// 			pallet_cf_flip::Reserve::<Runtime>::insert(
// 				pallet_cf_flip::ONCHAIN_FLIP_TO_DISTRIBUTE_RESERVE_ID,
// 				RESERVE_AMOUNT,
// 			);

// 			let authority_count =
// 				pallet_cf_validator::CurrentAuthorities::<Runtime>::get().len() as FlipBalance;
// 			let expected_per_authority_share = RESERVE_AMOUNT / authority_count;

// 			let estimate = reward_distribution_estimate();

// 			assert_eq!(estimate.epoch_index, epoch_index);
// 			assert_eq!(estimate.bond, pallet_cf_validator::Bond::<Runtime>::get());
// 			assert_eq!(estimate.pool_onchain_amount, RESERVE_AMOUNT);
// 			assert_eq!(estimate.pool_offchain_amount, 0);
// 			assert_eq!(estimate.per_authority_share, expected_per_authority_share);
// 			assert!(estimate
// 				.solo_validators
// 				.iter()
// 				.all(|v| v.reward == estimate.per_authority_share));

// 			let operator_estimate = estimate
// 				.operators
// 				.iter()
// 				.find(|o| o.account == operator)
// 				.expect("operator should have an estimate entry");

// 			assert_eq!(operator_estimate.commission_bps, 2000);
// 			assert_eq!(operator_estimate.num_authority_nodes, 1);
// 			assert_eq!(
// 				operator_estimate.total_delegator_stake,
// 				delegators.values().copied().sum::<FlipBalance>()
// 			);

// 			// Cross-check against the same splitting logic the runtime uses at epoch-end.
// 			let snapshot =
// 				pallet_cf_validator::DelegationSnapshots::<Runtime>::get(epoch_index, &operator)
// 					.expect("snapshot should exist");
// 			let expected: BTreeMap<AccountId, FlipBalance> = snapshot
// 				.distribute(expected_per_authority_share, estimate.bond)
// 				.map(|(account, amount)| (account.clone(), amount))
// 				.collect();

// 			assert_eq!(operator_estimate.operator_reward, expected[&operator]);
// 			for v in &operator_estimate.validators {
// 				assert_eq!(v.reward, expected[&v.account]);
// 			}
// 			for d in &operator_estimate.delegators {
// 				assert_eq!(d.reward, expected[&d.account]);
// 			}
// 		});
// }
