...
fn block_author_rewards_are_distributed_among_delegators() {
	...
	super::genesis::with_test_defaults()
		...
		.execute_with(|| {
			...
			pallet_cf_validator::DelegationCapacityFactor::<Runtime>::set(1_000_000u32);
			...
			let snapshot =
				pallet_cf_validator::DelegationResolver::<Runtime>::resolve_for_account(&operator)
					.expect("Snapshot should be registered on new epoch");
			assert_eq!(
				snapshot,
				pallet_cf_validator::DelegationResolver::<Runtime>::resolve_for_account(&auth)
					.expect("Snapshot should be registered on new epoch"),
			);
			assert!(
			...
			let total_delegators_pre_balance: FlipBalance =
				delegators.iter().map(|(account, _)| Flip::balance(account)).sum();
			...
			assert!(
				u16::abs_diff(
					PerU16::from_rational(auth_cut, total_auth_reward).deconstruct(),
					PerU16::from_rational(auth_pre_balance, total_pre_balance).deconstruct()
				) < 10,
				"Expected values differed by {} parts",
				u16::abs_diff(
					PerU16::from_rational(auth_cut, total_auth_reward).deconstruct(),
					PerU16::from_rational(auth_pre_balance, total_pre_balance).deconstruct()
				)
...
fn slashings_are_distributed_among_delegators() {
	...
	super::genesis::with_test_defaults()
		...
		.execute_with(|| {
			...
			assert!(
				u16::abs_diff(
					PerU16::from_rational(auth_slash, total_slashing).deconstruct(),
					PerU16::from_rational(auth_pre_balance, total_pre_balance).deconstruct()
				) < 10,
				"Expected values differed by {} parts",
				u16::abs_diff(
