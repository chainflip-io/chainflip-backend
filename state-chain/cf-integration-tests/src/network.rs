				...
				on_events!(
					...
					RuntimeEvent::EthereumThresholdSigner(
						...
						let _result = state_chain_runtime::EthereumThresholdSigner::signature_success(
							RuntimeOrigin::none(),
							*ceremony_id,
							self.eth_threshold_signer.borrow().sign_with_key(key_id.clone().into(), payload.as_fixed_bytes()),
						);
					}
						...
						pallet_cf_threshold_signature::Event::ThresholdSignatureRequest {
							...
							payload}) => {
								let _result = state_chain_runtime::PolkadotThresholdSigner::signature_success(
									RuntimeOrigin::none(),
									*ceremony_id,
									self.dot_threshold_signer.borrow().sign_with_key(key_id.clone().into(), &(payload.clone().0)),
								);
					}
						...
						pallet_cf_threshold_signature::Event::ThresholdSignatureRequest {
							...
							payload}) => {
								let _result = state_chain_runtime::BitcoinThresholdSigner::signature_success(
									RuntimeOrigin::none(),
									*ceremony_id,
									vec![self.btc_threshold_signer.borrow().sign_with_key(key_id.clone(), &payload[0].1)],
								);
					}
					RuntimeEvent::Validator(
