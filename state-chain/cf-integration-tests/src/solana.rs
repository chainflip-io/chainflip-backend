...
use cf_chains::{
	...
	sol::{
		...
		SolPubkey, SolanaCrypto, VaultSwapOrDepositChannelId,
	},
	...
	witness_call(RuntimeCall::SolanaIngressEgress(
		pallet_cf_ingress_egress::Call::process_deposits {
			deposit_witnesses: vec![DepositWitness {
				...
				deposit_details: VaultSwapOrDepositChannelId::Channel(deposit_address),
			}],
			block_height: 0,
		},
	));
...
fn vault_swap_deposit_witness(
	...
	VaultDepositWitness {
		...
		deposit_details: VaultSwapOrDepositChannelId::VaultSwapAccount((SolAddress([2u8; 32]), 0)),
