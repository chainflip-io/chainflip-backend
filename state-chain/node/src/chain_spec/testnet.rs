...
pub const ENV: StateChainEnvironment = StateChainEnvironment {
	...
	sol_address_lookup_table_account: (
		SolAddress(bs58_array("G6cZT2goBpUzaqsvaNsCc9AxRJsuccZ1ViPfL3iST9pV")),
		[
...
pub fn extra_accounts() -> Vec<(AccountId, AccountRole, FlipBalance, Option<Vec<u8>>)> {
	vec![
		...
		(
			get_account_id_from_seed::<sr25519::Public>("LP_LENDING"),
			AccountRole::LiquidityProvider,
			100 * FLIPPERINOS_PER_FLIP,
			Some(b"Chainflip Testnet LP LENDING".to_vec()),
		),
		(
			get_account_id_from_seed::<sr25519::Public>("BROKER_API"),
			AccountRole::Broker,
			200 * FLIPPERINOS_PER_FLIP,
			Some(b"Chainflip Testnet Broker Api".to_vec()),
		),
		(
