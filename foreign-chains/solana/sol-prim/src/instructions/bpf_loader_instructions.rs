use crate::{consts::BPF_LOADER_UPGRADEABLE_ID, AccountMeta, Instruction, Pubkey};
use sp_std::vec;
...
pub enum UpgradeableLoaderInstruction {
	...
	///   - The ProgramData account will remain mutable by the loader only and holds the program
	///     data and authority information.  The ProgramData account's address is derived from the
	///     Program account's address and created by the DeployWithMaxDataLen instruction.
	///
	...
	///
	/// # Account references
	///   0. `[signer]` The payer account that will pay to create the ProgramData account.
	///   1. `[writable]` The uninitialized ProgramData account.
	///   2. `[writable]` The uninitialized Program account.
	///   3. `[writable]` The Buffer account where the program data has been written.  The buffer
	///      account's authority must match the program's authority
	///   4. `[]` Rent sysvar.
	///   5. `[]` Clock sysvar.
	...
	///   2. `[writable]` The Buffer account where the program data has been written.  The buffer
	///      account's authority must match the program's authority
	...
	///   2. `[]` The new authority, optional, if omitted then the program will not be upgradeable.
	SetAuthority,
	...
	///
	/// # Account references
	///   0. `[writable]` The account to close, if closing a program must be the ProgramData
	///      account.
	///   1. `[writable]` The account to deposit the closed account's lamports.
	///   2. `[signer]` The account's authority, Optional, required for initialized accounts.
	///   3. `[writable]` The associated Program account if the account to close is a ProgramData
	///      account.
	Close,
...
pub fn set_upgrade_authority(
	programdata_address: Pubkey, // Address derivation moved outside of this function
