...
// Return types from the OracleQueryHelperProgram::query_price_feeds instruction.
// They are not defined within the autogenerated ABI because it's manually returned
// via Anchor's `set_return_data` so we declare it manually.
#[derive(BorshDeserialize, Debug, Clone, PartialEq)]
pub struct PriceFeedResponse {
	pub query_timestamp: i64,
	pub results: Vec<PriceFeedData>,
}
...
impl TryFrom<Vec<u8>> for PriceFeedResponse {
	type Error = borsh::io::Error;
	fn try_from(data: Vec<u8>) -> Result<Self, borsh::io::Error> {
		PriceFeedResponse::try_from_slice(&data)
	}
}
