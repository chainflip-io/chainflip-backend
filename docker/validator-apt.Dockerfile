...
RUN echo "deb https://${APT_REPO_USERNAME}:${APT_REPO_PASSWORD}@apt.aws.chainflip.xyz/ci/${RELEASE}/${COMMIT_HASH}/ focal main" | tee /etc/apt/sources.list.d/chainflip.list
RUN apt-get update
