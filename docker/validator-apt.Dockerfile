...
RUN if [[ "$RELEASE" == "sandstorm" ]] ; then echo "deb https://${APT_REPO_USERNAME}:${APT_REPO_PASSWORD}@apt.aws.chainflip.xyz/ci/${COMMIT_HASH}/ focal main" | tee /etc/apt/sources.list.d/chainflip.list ; else echo "deb https://${APT_REPO_USERNAME}:${APT_REPO_PASSWORD}@apt.aws.chainflip.xyz/ci/ibiza/${COMMIT_HASH}/ focal main" | tee /etc/apt/sources.list.d/chainflip.list ; fi
RUN apt-get update
