...
impl<'a> fmt::Display for WithLink<'a> {
	fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
...
impl<'doc> TypeDoc<'doc> {
	...
	pub fn from_ref<'new>(schema: &'new Value) -> TypeDoc<'new> {
		TypeDoc {
			name: TypeRef::new(
				schema
					.get("$ref")
			...
			return TypeVariant::Object(
				properties
					...
					.map(|(name, field)| {
						FieldDoc::new(
							name,
							requireds.map_or(false, |requireds| {
...
impl SchemaAnalyzer {
	fn analyze<'a>(schema: &'a Value) -> Result<ApiDocs<'a>, Box<dyn Error>> {
		...
		let mut seen = methods
			.iter()
			.map(|MethodDef { request, response, .. }| [&request.name[..], &response.name[..]])
			.flatten()
			.collect::<BTreeSet<_>>();
