...
enum WitnessInformation {
	...
	VaultDeposit {
		...
		output_asset: Option<cf_chains::assets::any::Asset>,
		amount: NumberOrHex,
		destination_address: Option<TrackerAddress>,
	...
	<T::TargetChain as Chain>::DepositDetails: IntoDepositDetailsAnyChain,
{
	fn into_witness_information(
		...
		WitnessInformation::VaultDeposit {
			...
			destination_address: self.destination_address.map(Into::into),
		...
		assert_ok!(
			save_vault_deposit_witness(
				...
				> {
					...
					output_asset: Some(chainflip_api::primitives::Asset::Flip),
					destination_address: Some(cf_chains::address::EncodedAddress::Dot([0; 32])),
