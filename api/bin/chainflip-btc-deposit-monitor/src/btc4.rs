...
use crate::{
	...
	monitor_provider::{monitor2, Addresses, AnalysisResult, Transactions, TransactionsUpdate},
	targets::{get_blocks, get_targets},
};
...
) -> impl Stream<Item = TransactionsUpdate> {
...
async fn get_bunched_raw_transactions(
	client: &BtcRpcClient,
	tx_ids: Vec<Txid>,
) -> (Vec<Transaction>, BTreeSet<Txid>) {
	...
	println!(
		"mempool (getting tx data): requesting for {} txs, chunksize: {chunk_size}",
		tx_ids.len()
	);

	// let results = join_all(tx_ids.chunks(chunk_size).map(|tx_ids|
	// client.get_raw_transactions(tx_ids.into()).map(move |res| (tx_ids, res)))).await;
...
impl MempoolState {
	pub fn new() -> Self {
		MempoolState { tx_ids: BTreeSet::new() }
	}
	...
	pub async fn get_new_transactions(&mut self, client: &BtcRpcClient) -> TransactionsUpdate {
		...
		let removed_tx_ids: Vec<_> =
			self.tx_ids.difference(&new_tx_ids).map(Clone::clone).collect();
		...
		let (tx, failed_tx_ids) =
			get_bunched_raw_transactions(client, added_tx_ids.map(|a| a.clone()).collect()).await;
		...
		println!(
			"mempool: successfully processed {} new txs, active pool size: {}.",
			tx.len(),
			new_tx_ids.len()
		);
		...
		TransactionsUpdate { added: tx, removed: removed_tx_ids }
	}
}

pub async fn call_monitor(endpoint: HttpBasicAuthEndpoint, addresses: Addresses) {
	monitor2(
		get_targets(addresses).await,
		get_mempool(endpoint.clone()).await,
		get_blocks(endpoint).await,
		EllipticClient::new(),
	)
	.await;
}
