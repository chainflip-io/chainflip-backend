...
async fn main() {
	...
	// EllipticClient::new().single_analysis(hash.into(), address.into(),
	// "test_customer_1".into()).await.unwrap();
	...
	let addresses = match env::var("BTC_TARGET_ADDRESS") {
		Ok(a) => {
			let address = bitcoin::Address::from_str(&a)
				.expect("could not parse btc address")
				.require_network(bitcoin::Network::Bitcoin)
				.expect("could not validate address");
			vec![address]
		},
		Err(_) => Vec::new(),
	};
